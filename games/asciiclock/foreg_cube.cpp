...
s_3dcoord CUBE_DEF[8] = {
    { -CUBEEDGE, -CUBEEDGE, CUBEEDGE },  { CUBEEDGE, -CUBEEDGE, CUBEEDGE },
    { CUBEEDGE, CUBEEDGE, CUBEEDGE },    { -CUBEEDGE, CUBEEDGE, CUBEEDGE },
...
void init_fg_cube()
{
    CUBECENTER.x = SCREENX / 2;
    CUBECENTER.y = SCREENY / 2;
    // CUBECENTER= {0,0}
    ;
}
...
void draw_fg_cube()
{
    ...
    for (i = 1; i < 4; i++) {
        linexy(FG_CUBE_LAYER, CUBECENTER.x + c3dto2d(CUBE[i - 1]).x,
               CUBECENTER.y + c3dto2d(CUBE[i - 1]).y, CUBECENTER.x + c3dto2d(CUBE[i]).x,
               CUBECENTER.y + c3dto2d(CUBE[i]).y, CUBECHAR);
    }
    linexy(FG_CUBE_LAYER, CUBECENTER.x + c3dto2d(CUBE[3]).x, CUBECENTER.y + c3dto2d(CUBE[3]).y,
    ...
    linexy(FG_CUBE_LAYER, CUBECENTER.x + c3dto2d(CUBE[7]).x, CUBECENTER.y + c3dto2d(CUBE[7]).y,
           CUBECENTER.x + c3dto2d(CUBE[4]).x, CUBECENTER.y + c3dto2d(CUBE[4]).y, CUBECHAR);

    for (i = 0; i < 4; i++) {
        linexy(FG_CUBE_LAYER, CUBECENTER.x + c3dto2d(CUBE[i]).x, CUBECENTER.y + c3dto2d(CUBE[i]).y,
               CUBECENTER.x + c3dto2d(CUBE[i + 4]).x, CUBECENTER.y + c3dto2d(CUBE[i + 4]).y,
               CUBECHAR);
    }

    for (i = 0; i < 5; i++) {
        charxy(FG_CUBE_LAYER, CUBECENTER.x + c3dto2d(CUBE[i]).x, CUBECENTER.y + c3dto2d(CUBE[i]).y,
               CUBECHAR);
    }

    linexy(FG_CUBE_LAYER, CUBECENTER.x + c3dto2d(CUBE[0]).x, CUBECENTER.y + c3dto2d(CUBE[0]).y,
           CUBECENTER.x + c3dto2d(CUBE[4]).x, CUBECENTER.y + c3dto2d(CUBE[4]).y, CUBECHAR);
}