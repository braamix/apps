...
const int MATRIX_BGLAYER = 0, MATRIX_FGLAYER = 1;

short unsigned int MATRIX_BGCOL = C_GREEN, MATRIX_FGCOL = C_LGREEN;
int MATRIX_MAXSPD = 5, MATRIX_MAXLEN = SCREENY / 2, MATRIX_MINLEN = SCREENY / 5;
...
{
    MATRIXCHRS[x].chr = { (char)(clock_rand() % 93 + 33), MATRIX_FGCOL, 8, false, false, false };
    MATRIXCHRS[x].spd = clock_rand() % MATRIX_MAXSPD;
    MATRIXCHRS[x].y =
        0 - clock_rand() % SCREENY / 2; // always start with a negative number -> it will start
                                        // above the screen, and flows down from there
    MATRIXCHRS[x].counter = 0;
    MATRIXCHRS[x].len     = clock_rand() % (int)(SCREENY / 2) + (int)(SCREENY / 5);
}
...
void calc_bg_matrix()
{
    ...
    for (x = 0; x < SCREENX; x++) {
        ...
        if (MATRIXCHRS[x].counter > MATRIXCHRS[x].spd) {
            ...
            mychr = { (char)(clock_rand() % 93 + 33), MATRIX_BGCOL, 8, false, false, false };
            ...
            MATRIXCHRS[x].chr = {
                (char)(clock_rand() % 93 + 33), MATRIX_FGCOL, 8, false, false, false
            };
