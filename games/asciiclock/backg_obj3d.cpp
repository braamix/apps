...
s_simplechar OBJ3DCHARS[MAXOBJ3DTYPE] = {
    { 46, C_DGRAY, 8, false, false, true }, { 46, C_DGRAY, 8, false, false, true },
    { 42, C_DGRAY, 8, false, false, true }, { 42, C_DGRAY, 8, false, false, true },
    { 42, C_GRAY, 8, false, false, true },  { 42, C_GRAY, 8, false, false, true },
    { 43, C_GRAY, 8, false, false, true },  { 43, C_GRAY, 8, false, false, true },
    { 43, C_WHITE, 8, false, false, true }, { 120, C_WHITE, 8, false, false, true },
...
s_3dcoord OBJS3D[MAXOBJ3D], // to store the original x,y,z coords
    OBJS3DCALC[MAXOBJ3D];   // store the rotated coords for every frame
                            // didn't use the same array, because rounding errors will mess up the
                            // original coords quickly
...
void init_bg_obj3d()
{
    int i, j, circlepointsno, circleslicesno;
    // clear bg layers
    OBJ3DCENTER.x = SCREENX / 2;
    OBJ3DCENTER.y = SCREENY / 2;
    // OBJ3DCENTER={  0, 0};
    for (i = 0; i <= 1; i++) {
        clearlayer(i, OBJ3DCLEAR);
    }

    OBJ3DOBJECT = clock_rand() % MAXOBJ3DOBJECTS;
    ...
    case 0: // sphere-like object
        ...
        for (i = 0; i < MAXOBJ3D; i++) {
            // random cooord
            OBJS3D[i].x = ((clock_rand() % (MAXOBJ3DDIST - MINOBJ3DDIST)) + MINOBJ3DDIST) *
                          OBJ3DNEGPOS[clock_rand() % 2];
            OBJS3D[i].y = ((clock_rand() % (MAXOBJ3DDIST - MINOBJ3DDIST)) + MINOBJ3DDIST) *
                          OBJ3DNEGPOS[clock_rand() % 2];
            OBJS3D[i].z = ((clock_rand() % (MAXOBJ3DDIST - MINOBJ3DDIST)) + MINOBJ3DDIST) *
                          OBJ3DNEGPOS[clock_rand() % 2];
            // and rotated on random axis
            OBJS3D[i] = c3drotate(clock_rand() % 3 + 1, clock_rand() % 360, OBJS3D[i]);
        }
        break;
    case 1: // plane
        ...
        for (i = 0; i < MAXOBJ3D; i++) {
            OBJS3D[i].x = 0;
            OBJS3D[i].y = (clock_rand() % MAXOBJ3DDIST * 2) - MAXOBJ3DDIST;
            OBJS3D[i].z = (clock_rand() % MAXOBJ3DDIST * 2) - MAXOBJ3DDIST;
        }
        break;
    case 2: // ring
        ...
        for (i = 0; i < MAXOBJ3D; i++) {
            OBJS3D[i].x =
                ((clock_rand() % MINOBJ3DDIST) - MINOBJ3DDIST / 2) * OBJ3DNEGPOS[clock_rand() % 2];

            OBJS3D[i].y = ((clock_rand() % (MAXOBJ3DDIST - MINOBJ3DDIST)) + MINOBJ3DDIST) *
                          OBJ3DNEGPOS[clock_rand() % 2];
            OBJS3D[i].z = ((clock_rand() % (MAXOBJ3DDIST - MINOBJ3DDIST)) + MINOBJ3DDIST) *
                          OBJ3DNEGPOS[clock_rand() % 2];
            OBJS3D[i]   = c3drotate(1, clock_rand() % 360, OBJS3D[i]);
        }
        break;
    ...
    case 3: // sphere
        ...
        for (i = 0; i < circlepointsno; i++) {
            ...
            /*
            OBJS3D[i]=c3drotate(1, clock_rand() % 360, OBJS3D[i]);
            OBJS3D[i]=c3drotate(2, clock_rand() % 360, OBJS3D[i]);
            OBJS3D[i]=c3drotate(3, clock_rand() % 360, OBJS3D[i]);
            */
        }
    ...
    case 4: // cube
        ...
        for (i = 0; i < MAXOBJ3D; i++) {
            ...
            OBJS3D[i].y =
                ((clock_rand() % MAXOBJ3DDIST * 2) - MAXOBJ3DDIST) * OBJ3DNEGPOS[clock_rand() % 2];
            OBJS3D[i].z =
                ((clock_rand() % MAXOBJ3DDIST * 2) - MAXOBJ3DDIST) * OBJ3DNEGPOS[clock_rand() % 2];
            OBJS3D[i] = c3drotate(clock_rand() % 2 + 2, (clock_rand() % 4) * 90, OBJS3D[i]);
        }
        break;
    case 5: // cube2
        MAXOBJ3DDIST = round(SCREENY / 3 * 2);
        MINOBJ3DDIST = MAXOBJ3DDIST - 1;
        for (i = 0; i < MAXOBJ3D; i++) {
            OBJS3D[i].x = MAXOBJ3DDIST * OBJ3DNEGPOS[clock_rand() % 2];

            OBJS3D[i].y = (clock_rand() % MAXOBJ3DDIST) * OBJ3DNEGPOS[clock_rand() % 2];
            OBJS3D[i].z = MAXOBJ3DDIST;
            OBJS3D[i]   = c3drotate(clock_rand() % 3 + 1, (clock_rand() % 4) * 90, OBJS3D[i]);
            OBJS3D[i]   = c3drotate(clock_rand() % 3 + 1, (clock_rand() % 4) * 90, OBJS3D[i]);
            OBJS3D[i]   = c3drotate(clock_rand() % 3 + 1, (clock_rand() % 4) * 90, OBJS3D[i]);
        }
        break;
...
void draw_bg_obj3d()
{
    ...
    for (i = 0; i < MAXOBJ3D; i++) {
        ...
        int dist =
            (int)((OBJS3DCALC[i].z + MAXOBJ3DDIST / 2) / ((MAXOBJ3DDIST * 2 / MAXOBJ3DTYPE) - 1));
        if (dist < 0) {
            dist = 0;
        }
        if (dist > MAXOBJ3DTYPE - 1) {
            dist = MAXOBJ3DTYPE - 1;
        }
        // draw the point
        charxy(OBJ3DLAYER, OBJ3DCENTER.x + c3dto2d(OBJS3DCALC[i]).x,
               OBJ3DCENTER.y + c3dto2d(OBJS3DCALC[i]).y, OBJ3DCHARS[dist]);
        //}
    }
}
