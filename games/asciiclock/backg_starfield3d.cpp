...
s_simplechar STAR3DCHARS[MAXSTAR3DTYPE] = {
    { 32, C_DGRAY, 8, false, false, true },  { 46, C_DGRAY, 8, false, false, true },
    { 42, C_DGRAY, 8, false, false, true },  { 42, C_GRAY, 8, false, false, true },
    { 43, C_GRAY, 8, false, false, true },   { 43, C_WHITE, 8, false, false, true },
    { 120, C_WHITE, 8, false, false, true }, { 88, C_WHITE, 8, false, false, true },
    { 88, C_WHITE, 8, false, false, true },

    { 46, C_DGRAY, 8, false, false, true }, // these will never appear, because we don't draw
                                            // anything that has z>0
    { 42, C_DGRAY, 8, false, false, true },  { 42, C_GRAY, 8, false, false, true },
    { 43, C_GRAY, 8, false, false, true },   { 43, C_WHITE, 8, false, false, true },
    { 120, C_WHITE, 8, false, false, true }, { 88, C_WHITE, 8, false, false, true },
    { 88, C_WHITE, 8, false, false, true }
...
void init_bg_star3d()
{
    int i;
    // clear bg layers
    STAR3DCENTER.x = SCREENX / 2;
    STAR3DCENTER.y = SCREENY / 2;
    // STAR3DCENTER={  0, 0};
    for (i = 0; i <= 1; i++) {
    ...
    for (i = 0; i < MAXSTAR3D; i++) {
        // random cooord
        STARS3D[i].x = ((clock_rand() % (MAXSTAR3DDIST - MINSTAR3DDIST)) + MINSTAR3DDIST) *
                       NEGPOS[clock_rand() % 2];
        STARS3D[i].y = ((clock_rand() % (MAXSTAR3DDIST - MINSTAR3DDIST)) + MINSTAR3DDIST) *
                       NEGPOS[clock_rand() % 2];
        STARS3D[i].z = ((clock_rand() % (MAXSTAR3DDIST - MINSTAR3DDIST)) + MINSTAR3DDIST) *
                       NEGPOS[clock_rand() % 2];
        // and rotated on random axis
        STARS3D[i] = c3drotate(clock_rand() % 3 + 1, clock_rand() % 360, STARS3D[i]);
    }
...
void draw_bg_star3d()
{
    ...
    for (i = 0; i < MAXSTAR3D; i++) {
        if (STARS3DCALC[i].z < 0) { // if its far from camera then draw
            ...
            int dist = (int)((STARS3DCALC[i].z + MAXSTAR3DDIST) /
                             ((MAXSTAR3DDIST * 2 / MAXSTAR3DTYPE) - 1));
            if (dist < 0) {
                dist = 0;
            }
            if (dist > MAXSTAR3DTYPE - 1) {
                dist = MAXSTAR3DTYPE - 1;
            }
            // draw the star
            charxy(STAR3DLAYER, STAR3DCENTER.x + c3dto2d(STARS3DCALC[i]).x,
                   STAR3DCENTER.y + c3dto2d(STARS3DCALC[i]).y, STAR3DCHARS[dist]);
        }
    }
}
