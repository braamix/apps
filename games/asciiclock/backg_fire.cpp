...
s_simplechar FIRECHARS[MAXFIRECOLORS][MAXFIRECHARS] = {
    { { 32, 1, 8, false, false, false }, // dummy char 0 is for nothing
      { 32, 0, 8, false, false, false },         { 32, 0, 8, false, false, false },
      { 32, 0, 8, false, false, false },         { 32, 0, 8, false, false, false },

      { 46, C_DGRAY, 8, false, false, true },    { 43, C_RED, 8, false, false, true },
      { 120, C_YELLOW, 8, false, false, true },  { 88, C_YELLOW, 8, false, false, true },
      { 120, C_LYELLOW, 8, false, false, true }, { 88, C_LYELLOW, 8, false, false, true },
      { 120, C_LGREEN, 8, false, false, true },  { 88, C_LGREEN, 8, false, false, true },
      { 120, C_WHITE, 8, false, false, true },   { 88, C_WHITE, 8, false, false, true },
      { 120, C_WHITE, 8, false, false, true },   { 88, C_WHITE, 8, false, false, true },
      { 120, C_WHITE, 8, false, false, true },   { 88, C_WHITE, 8, false, false, true },
      { 88, C_WHITE, 8, false, false, true },    { 88, C_WHITE, 8, false, false, true },
      { 88, C_WHITE, 8, false, false, true },    { 88, C_WHITE, 8, false, false, true }
...
{
    int x, y;
    x                   = float(clock_rand() % SCREENX);
    y                   = float(SCREENY - clock_rand() % SCREENY / 4);
...
void draw_random_bottom()
{
    int i;
    for (i = 0; i <= SCREENX; i++) {
        FIREBUFFER[0][SCREENY][i] = clock_rand() % MAXFIRECHARS;
    }
}
...
__attribute__((noinline)) void calc_bg_fire()
{
    int x, y;
    int average = 0;
    ...
    for (y = 1; y <= SCREENY; y++) {
        for (x = 1; x <= SCREENX - 1; x++) {
            average =
                floor((FIREBUFFER[0][y - 1][x] + FIREBUFFER[0][y][x - 1] + FIREBUFFER[0][y][x] +
                       FIREBUFFER[0][y][x + 1] + FIREBUFFER[0][y + 1][x]) /
                      5);
            if (average > MAXFIRECHARS) {
                average = MAXFIRECHARS;
            }
