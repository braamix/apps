...
[archivers/bzip2](archivers/bzip2/), Julian Seward bzip2 1.0.8 — gzip's PORT
shape on `braam::bzip2` alone; upstream's `BZFILE` layer becomes `bz_stream`
over `b_fread`/`b_fwrite`; and
...
335 files of `Lib/` ship as `lib/`, with
...
`web/kernel.wasm` and `web/rootfs.zip` of the same release — so the kernel under
test is the one the binaries were stamped against. `make SDK=<prefix>` is the
one knob: to test on a local core, `cmake --install ../braam-core/build
--prefix <prefix>` and name that. [test/sdk.mjs](test/sdk.mjs) is the one place
a test learns where the SDK is — `BRAAM_SDK` when set, otherwise the fetched
one — and exports `HARNESS`, `KERNEL` and `ROOTFS`, so a test run by hand with
`node` finds the same files. The SDK's manual, §3.3, is the harness's API.
...
the SDK's `HARNESS` with the unmodified rootfs and serve
...
From core's own tree, `test/system/abi.mjs` asserts all of it for any binary
...
imports the SDK's `HARNESS` directly — `test/run.mjs` is not
