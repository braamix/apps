...
local Task<int> find_next_signature OF((FILE * f));
local Task<int> find_signature OF((FILE *, ZCONST char *));
local int is_signature OF((ZCONST char *, ZCONST char *));
local Task<int> at_signature OF((FILE *, ZCONST char *));
...
// ------------------------------------------------------- reading an archive
//
// The scanners, the local header reader, and zipcopy. Everything here
// reaches the archive, so all of it is Task-returning but the three sort
// callbacks and the signature compare.
...
local Task<void> read_Unicode_Path_entry(struct zlist far *pZipListEntry)
{
    char *pTemp;
    char *UPath;
    char *iname;
    ...
    pTemp += 1;
    if (Version > 1) {
        co_await zipwarn("Unicode Path Extra Field version > 1 - skipping", pZipListEntry->oname);
        co_return;
    }
    ...
    if ((iname = (char *)malloc(strlen(pZipListEntry->iname) + 1)) == NULL) {
        {
            co_await ziperr_msg(ZE_MEM, "write Unicode");
            co_return;
        }
    }
    ...
    if (chksum != iname_chksum) {
        ...
        if (unicode_mismatch == 1) {
            ...
            {
                co_await ziperr_msg(ZE_FORM, "Unicode path error");
                co_return;
            }
        }
        co_return;
    ...
    if (ULen == 0) {
        ...
        if ((UPath = (char *)malloc(ULen + 1)) == NULL) {
            co_return;
        }
...
local Task<void> read_Unicode_Path_local_entry(struct zlist far *pZipListEntry)
{
    char *pTemp;
    char *UPath;
    char *iname;
...
Task<zoff_t> ffile_size(FILE *file)
{
    int sts;
    usize siz;
    zoff_t ofs;
...
Task<int> readlocal(struct zlist far **localz, struct zlist far *z)
{
    char buf[LOCHEAD + 1];
    ...
    if (in_file == NULL) {
        while ((in_file = co_await b_fopen(split_path, FOPR)) == NULL) {
            ...
            if (co_await ask_for_split_read_path(start_disk) != ZE_OK) {
                co_return ZE_ABORT;
            }
            free(split_path);
    ...
    if ((locz->name = (char *)malloc(locz->nam + 1)) == NULL)
        co_return ZE_MEM;
    ...
    if (locz->lflg != z->flg) {
        co_await zipwarn("Local Entry Flag does not match CD: ", z->iname);
    }
    if (locz->crc != z->crc) {
        co_await zipwarn("Local Entry CRC does not match CD: ", z->iname);
    }
...
local Task<int> find_next_signature(FILE *f)
{
    int m;
    // zoff_t here;
...
local Task<int> find_signature(FILE *f, ZCONST char *signature)
{
    int i;
    char sig[4];
...
local Task<int> at_signature(FILE *f, ZCONST char *signature)
{
    int i;
    extent m;
    char sig[4];
...
local Task<int> scanzipf_fixnew(void)
...
#define FIXSCAN_BUFSIZE ENDHEAD
    ...
    uzoff_t offset         = 0; // location after co_return from seek
    ...
    if (plen >= 4 && in_path_ext[0] == '.' && toupper(in_path_ext[1]) == 'Z' &&
        in_path_ext[2] >= '0' && in_path_ext[2] <= '9' && in_path_ext[3] >= '0' &&
        in_path_ext[3] <= '9' && (plen == 4 || (in_path_ext[4] >= '0' && in_path_ext[4] <= '9'))) {
        ...
        if (!co_await find_signature(in_file, "PK\05\06")) {
            ...
            if (s < ENDHEAD) {
                ...
                if (total_disks < 0x10000 && in_cd_start_disk > total_disks) {
                    co_await zipwarn("End record (EOCDR) has bad disk numbers - ignoring EOCDR",
                                     "");
                    total_disks = 0;
                } else {
                ...
                if (current_in_offset == 8 && is_signature(sigbuf, "PK\03\04")) {
                    ...
                    if (asked) {
                        if (errbuf[0] == 'y' || errbuf[0] == 'Y') {
                            total_disks = 1;
                            co_await zipmessage("  Assuming single-disk archive", "");
                        }
                    }
                }
            }
    ...
    co_await zipmessage("Scanning for entries...", "");
    ...
    for (current_in_disk = 0; current_in_disk < total_disks; current_in_disk++) {
        ...
        if (in_file == NULL) {
            ...
            while ((in_file = co_await b_fopen(split_path, FOPR)) == NULL) {
                ...
                result = co_await ask_for_split_read_path(current_in_disk);
                if (result == ZE_ABORT) {
        ...
        while (co_await find_next_signature(in_file)) {
            ...
            if (is_signature(sigbuf, "PK\05\06")) {
                ...
                if (in_central_directory == 0) {
                    co_await zipmessage("Central Directory found...", "");
                ...
                if ((cz->iname = (char *)malloc(cz->nam + 1)) == NULL ||
                ...
                if (total_disks == 1) {
                    ...
                    co_await zipmessage(
                        "  Found spanning marker - expected as this is split (multi-disk) "
                        "archive...",
                        "");

                } else {
                    co_await zipmessage("  Found spanning marker - could be split archive...", "");
                }
...
local Task<int> scanzipf_regnew(void)
...
#define SCAN_BUFSIZE EC64LOC
    ...
    if (!co_await find_signature(in_file, "PK\05\06")) {
        ...
        if (fix == 1) {
            ...
            co_await zipwarn("(If downloaded, was binary mode used?  If not, the", "");
            co_await zipwarn(" archive may be scrambled and not recoverable)", "");
            co_await zipwarn("Can't use -F to fix (try -FF)", "");
        } else {
            co_await zipwarn("missing end signature--probably not a zip file (did you", "");
            co_await zipwarn("remember to use binary mode when you transferred it?)", "");
            co_await zipwarn("(if you are trying to read a damaged archive try -F)", "");
        }
        co_return ZE_FORM;
    }
    ...
    if (total_disks != 1) {
        ...
        if (adjust) {
            co_await zipwarn("Adjusting split archives not yet supported", "");
            co_return ZE_FORM;
        }
    ...
    if ((fix == 1 && total_disks == 1) || adjust) {
        ...
        if (in_cd_start_offset != 0xFFFFFFFF && cd_total_size != 0xFFFFFFFF) {
            ...
            if (co_await at_signature(in_file, "PK\06\07")) {
                ...
                if (total_disks != 1) {
                    co_await zipwarn("Adjusting split archives not supported:  ", in_path);
                    co_await zipwarn("(try -FF)", "");
                    co_return ZE_FORM;
                }
    ...
    if (co_await at_signature(in_file, "PK\06\07")) {
        ...
        if (z64eocdr_disk != total_disks - 1) {
            ...
            while ((in_file = co_await b_fopen(split_path, FOPR)) == NULL) {
                ...
                if (co_await ask_for_split_read_path(z64eocdr_disk) != ZE_OK) {
                    co_return ZE_ABORT;
                }
                free(split_path);
                split_path = get_in_split_path(in_path, z64eocdr_disk);
        ...
        if (!co_await at_signature(in_file, "PK\06\06")) {
            ...
            if (co_await find_next_signature(in_file) && is_signature(sigbuf, "PK\06\06")) {
                ...
                if (fix == 1) {
                    ...
                    co_await zipwarn("Zip64 End Of Central Directory Record not found:  ", in_path);
                }
                co_return ZE_FORM;
            }
        }
        ...
        if (version_needed > 46) {
            ...
            if (fix == 1)
                co_await zipwarn("If -F fails try -FF to try to salvage something", "");
            else if (fix == 2)
    ...
    for (current_in_disk = in_cd_start_disk; current_in_disk < total_disks; current_in_disk++) {
        // get the path for this disk
        ...
        while (co_await find_next_signature(in_file)) {
            ...
            if ((z->iname = (char *)malloc(z->nam + 1)) == NULL ||
            ...
            if (unicode_mismatch != 3) {
                if (z->flg & UTF8_BIT) {
                    ...
                    // path is UTF-8
                    if ((z->uname = (char *)malloc(strlen(z->iname) + 1)) == NULL) {
                        co_await zipwarn("could not allocate memory: scanzipf_reg", "");
                        co_return ZE_MEM;
                    }
                    strcpy(z->uname, z->iname);
                    ...
                    if (iname) {
                        ...
                        co_await zipwarn("illegal UTF-8 name: ", z->uname);
                } else {
                    // check for UTF-8 path extra field
                    co_await read_Unicode_Path_entry(z);
                }
            ...
            if (z->zname == NULL)
                co_return ZE_MEM;
            if ((z->name = (char *)malloc(strlen(z->zname) + 1)) == NULL) {
                co_await zipwarn("could not allocate memory: scanzipf_reg", "");
                co_return ZE_MEM;
            }
            ...
            if (unicode_mismatch != 3) {
                if (z->uname) {
                    ...
                    if (name == NULL) {
                        ...
                        if ((name = (char *)malloc(strlen(z->iname) + 1)) == NULL) {
                            co_await zipwarn("could not allocate memory: scanzipf_reg", "");
                            co_return ZE_MEM;
                        }
                        strcpy(name, z->iname);
                    ...
                    if ((z->zuname = (char *)malloc(strlen(name) + 1)) == NULL) {
                        co_await zipwarn("could not allocate memory: scanzipf_reg", "");
                        co_return ZE_MEM;
                    }
                    ...
                    if (unicode_escape_all) {
                        ...
                        if (ouname)
                            ...
                            if ((z->ouname = (char *)malloc(strlen(name) + 1)) == NULL) {
                                co_await zipwarn("could not allocate memory: scanzipf_reg", "");
                                co_return ZE_MEM;
                            }
...
Task<int> zipcopy(struct zlist far *z)
{
    ...
    if (fix != 2) {
        ...
        if (in_file == NULL) {
            while ((in_file = co_await b_fopen(split_path, FOPR)) == NULL) {
                ...
                r = co_await ask_for_split_read_path(start_disk);
                if (r == ZE_ABORT) {
                    // user abort
                    co_return ZE_ABORT;
        ...
        if (unicode_mismatch != 3) {
            if (z->flg & UTF8_BIT) {
                ...
                if (iname == NULL) {
                    ...
                    if ((localz->iname = (char *)malloc(strlen(localz->uname) + 1)) == NULL) {
                        co_return ZE_MEM;
                    }
                ...
                // check for UTF-8 path extra field
                co_await read_Unicode_Path_local_entry(localz);
            }
        }
    }
    ...
    if (fix == 2) {
        ...
        // Compare localz to z
        if (localz->ver != z->ver) {
            co_await zipwarn("Local Version Needed To Extract does not match CD: ", z->iname);
        }
        ...
        if (fix != 3 && strcmp(localz->iname, z->iname) != 0) {
            co_await zipwarn("Local Entry name does not match CD: ", z->iname);
        }
    ...
    if (fix == 3) {
        ...
        // update local name
        free(localz->iname);
        if ((localz->iname = (char *)malloc(strlen(z->iname) + 1)) == NULL) {
            co_await zipwarn("out of memory in zipcopy", "");
            co_return ZE_MEM;
        }
    ...
    // Output name
    if (fix == 2) {
        if ((z->oname = (char *)malloc(strlen(localz->iname) + 1)) == NULL) {
            co_return ZE_MEM;
        }
    ...
    if (fix == 2 && localz->lflg & 8)
        ...
        r = co_await bfcopy((uzoff_t)-2);
    else
        r = co_await bfcopy(localz->siz);
    ...
    if (r == ZE_EOF || skip_this_disk) {
        // missing disk
        co_await zipwarn("aborting: ", z->oname);
    ...
    if (fix == 2 && z->flg & 8) {
        ...
        if (des_good) {
            ...
            co_await zipwarn("no end of stream entry found: ", z->oname);
    ...
    if (z->flg & 8) {
        co_await putextended(localz);
    }

    // now can close the split if local header on previous split
    if (split_method == 1 && current_local_disk != current_disk) {
        co_await close_split(current_local_disk, current_local_file, current_local_tempname);
        current_local_file = NULL;
        free(current_local_tempname);
    }
    ...
    if (fix == 2) {
        ...
        z->cextra = NULL;
        if (localz->ext) {
            if ((z->cextra = (char *)malloc(localz->ext + 1)) == NULL) {
                co_return ZE_MEM;
            }
        ...
        if ((z->zname = (char *)malloc(localz->nam + 1)) == NULL) {
            co_return ZE_MEM;
        }
