// Lzip decompressor — fd output, memory input (from FreeBSD unlz.c).
#include "braam.h"
...
static bool lz_st_is_char(int st)
{
    return st < 7;
}

static int lz_st_get_char(int st)
{
    return lz_st_next[st];
}

static int lz_st_get_match(int st)
{
    return st < 7 ? 7 : 10;
}

static int lz_st_get_rep(int st)
{
    return st < 7 ? 8 : 11;
}

static int lz_st_get_short_rep(int st)
{
    return st < 7 ? 9 : 11;
}
...
static void lz_crc_update(uint32_t *crc, const uint8_t *buf, size_t len)
{
...
static int lz_rd_create(struct lz_range_decoder *rd, LzIn *in)
{
    rd->in    = in;
    rd->code  = 0;
    rd->range = ~0;
    for (int i = 0; i < 5; i++)
        rd->code = (rd->code << 8) | (uint8_t)lz_in_getc(rd->in);
    return (rd->in->i > rd->in->n) ? -1 : 0;
}

static unsigned lz_rd_decode(struct lz_range_decoder *rd, int num_bits)
{
    unsigned symbol = 0;
...
static unsigned lz_rd_decode_bit(struct lz_range_decoder *rd, int *bm)
{
    unsigned symbol;
...
static unsigned lz_rd_decode_tree(struct lz_range_decoder *rd, int *bm, int num_bits)
{
    unsigned symbol = 1;
...
static unsigned lz_rd_decode_tree_reversed(struct lz_range_decoder *rd, int *bm, int num_bits)
{
...
static unsigned lz_rd_decode_matched(struct lz_range_decoder *rd, int *bm, int match_byte)
{
    unsigned symbol = 1;

    for (int i = 7; i >= 0; i--) {
        const unsigned match_bit = (match_byte >> i) & 1;
        const unsigned bit       = lz_rd_decode_bit(rd, &bm[symbol + (match_bit << 8) + 0x100]);
        symbol                   = (symbol << 1) | bit;
        if (match_bit != bit) {
            while (symbol < 0x100) {
                symbol = (symbol << 1) | lz_rd_decode_bit(rd, &bm[symbol]);
...
static unsigned lz_rd_decode_len(struct lz_range_decoder *rd, struct lz_len_model *lm,
                                 int pos_state)
{
    ...
    if (lz_rd_decode_bit(rd, &lm->choice2) == 0) {
        return LOW_SYMBOLS + lz_rd_decode_tree(rd, lm->bm_mid[pos_state], MID_BITS);
    }

    return LOW_SYMBOLS + MID_SYMBOLS + lz_rd_decode_tree(rd, lm->bm_high, HIGH_BITS);
}
...
static int lz_flush(struct lz_decoder *lz)
{
...
static void lz_destroy(struct lz_decoder *lz)
{
    free(lz->outbuf);
    free(lz->obuf);
}

static int lz_create_mem(struct lz_decoder *lz, LzIn *input, int fdout, int dict_size)
{
    ...
    lz->obuf = (uint8_t *)malloc(dict_size);
    if (lz->obuf == NULL)
        goto out;
...
static uint8_t lz_peek(const struct lz_decoder *lz, unsigned ahead)
{
...
static void lz_put(struct lz_decoder *lz, uint8_t b)
{
    lz->obuf[lz->pos++] = b;
...
static off_t lz_get_data_position(const struct lz_decoder *lz)
{
    return lz->ppos + lz->pos;
}

static unsigned lz_get_crc(const struct lz_decoder *lz)
{
    return lz->crc ^ 0xffffffffU;
}

static void lz_bm_init(int *a, size_t l)
{
...
static bool lz_decode_member(struct lz_decoder *lz)
{
    ...
    while (lz->fin->i < lz->fin->n) {
        ...
        if (lz_rd_decode_bit(rd, &bm_rep[0][state]) != 0) {
            ...
            if (lz_rd_decode_bit(rd, &bm_rep[1][state]) == 0) {
                // bit 4
                if (lz_rd_decode_bit(rd, &bm_len[state][pos_state]) == 0) {
                ...
                unsigned distance;
                // bit 4
                if (lz_rd_decode_bit(rd, &bm_rep[2][state]) == 0)
                    distance = rep[1];
                else {
                    // bit 5
                    if (lz_rd_decode_bit(rd, &bm_rep[3][state]) == 0)
                        distance = rep[2];
                    else {
                        distance = rep[3];
                        rep[3]   = rep[2];
            ...
            len   = MIN_MATCH_LEN + lz_rd_decode_len(rd, &rep_len_model, pos_state);
        } else {
            rep[3]              = rep[2];
            rep[2]              = rep[1];
            rep[1]              = rep[0];
            len                 = MIN_MATCH_LEN + lz_rd_decode_len(rd, &match_len_model, pos_state);
            const int len_state = MIN(len - MIN_MATCH_LEN, STATES - 1);
            ...
            if (rep[0] >= DIS_MODEL_START) {
                ...
                if (dis_slot < DIS_MODEL_END)
                    rep[0] +=
                        lz_rd_decode_tree_reversed(rd, &bm_dis[rep[0] - dis_slot], direct_bits);
                else {
                    rep[0] += lz_rd_decode(rd, direct_bits - DIS_ALIGN_BITS) << DIS_ALIGN_BITS;
                    rep[0] += lz_rd_decode_tree_reversed(rd, bm_align, DIS_ALIGN_BITS);
            ...
            if (rep[0] >= lz->dict_size || (rep[0] >= lz->pos && !lz->wrapped)) {
                lz_flush(lz);
                return false;
            }
        }
...
static unsigned lz_get_dict_size(unsigned char c)
{
...
Task<off_t> unlz(int fin, int fout, char *pre, size_t prelen, off_t *bytes_in)
{
    if (lz_crc[0] == 0)
        lz_crc_init();
