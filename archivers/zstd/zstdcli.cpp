...
/* One, always: UTIL_countLogicalCores() answers 1 here. */
...
#define WELCOME_MESSAGE                                                                          \
    "*** %s (%i-bit) %s, by %s ***\n", COMPRESSOR_NAME, (int)(sizeof(size_t) * 8), ZSTD_VERSION, \
        AUTHOR
...
static int g_displayLevel =
    ZSTD_DISPLAY_LEVEL_DEFAULT; /* 0 : no display,  1: errors,  2 : + result + interaction +
 ...
 * @return : 0 if the versions agree.  There is no dynamic library here, so
 * this can only disagree if the SDK moved under a stale object file. */
static int checkLibVersion(void)
{
...
{
    DISPLAY_F(f,
              "Compress or decompress the INPUT file(s); reads from STDIN if INPUT is `-` or not "
              "provided.\n\n");
    DISPLAY_F(f, "Usage: %s [OPTIONS...] [INPUT... | -] [-o OUTPUT]\n\n", programName);
    DISPLAY_F(f, "Options:\n");
    DISPLAY_F(f, "  -o OUTPUT                     Write output to a single file, OUTPUT.\n");
    DISPLAY_F(f,
              "  -c, --stdout                  Write to STDOUT (even if it is a console) and keep "
              "the INPUT file(s).\n");
    DISPLAY_F(f, "  -k, --keep                    Preserve INPUT file(s). [Default] \n");
    DISPLAY_F(f,
              "  --rm                          Remove INPUT file(s) after successful "
              "(de)compression to file.\n");
    ...
    DISPLAY_F(f,
              "  -#                            Desired compression level, where `#` is a number "
              "between 1 and %d;\n",
              ZSTDCLI_CLEVEL_MAX);
    DISPLAY_F(f,
              "                                lower numbers provide faster compression, higher "
              "numbers yield\n");
...
#endif
    DISPLAY_F(f,
              "  -D DICT                       Use DICT as the dictionary for compression or "
              "decompression.\n\n");
    DISPLAY_F(f,
              "  -f, --force                   Disable input and output checks. Allows overwriting "
              "existing files,\n");
    DISPLAY_F(f,
              "                                receiving input from the console, printing output "
              "to STDOUT, and\n");
    DISPLAY_F(f,
              "                                operating on links, block devices, etc. "
              "Unrecognized formats will be\n");
...
static void usageAdvanced(const char *programName)
{
    ...
    DISPLAYOUT(
        "  -v, --verbose                 Enable verbose output; pass multiple times to increase "
        "verbosity.\n");
    DISPLAYOUT(
    ...
    DISPLAYOUT("\n");
    DISPLAYOUT(
        "  --[no-]progress               Forcibly show/hide the progress counter. NOTE: Any "
        "(de)compressed\n");
    DISPLAYOUT(
        "                                output to terminal will mix with progress counter "
        "text.\n\n");
    ...
    DISPLAYOUT(
        "  --output-dir-mirror DIR       Store processed files in DIR, respecting original "
        "directory structure.\n");
#endif
    if (AIO_supported())
...
#ifndef ZSTD_NOCOMPRESS
    DISPLAYOUT(
        "  --[no-]check                  Add XXH64 integrity checksums during compression. "
        "[Default: Add, Validate]\n");
#ifndef ZSTD_NODECOMPRESS
    DISPLAYOUT(
        "                                If `-d` is present, ignore/validate checksums during "
        "decompression.\n");
#endif
#else
#ifdef ZSTD_NOCOMPRESS
    DISPLAYOUT(
        "  --[no-]check                  Ignore/validate checksums during decompression. [Default: "
        "Validate]");
#endif
    ...
    DISPLAYOUT(
        "  --ultra                       Enable levels beyond %i, up to %i; requires more "
        "memory.\n",
        ZSTDCLI_CLEVEL_MAX, ZSTD_maxCLevel());
    DISPLAYOUT(
        "  --fast[=#]                    Use to very fast compression levels. [Default: %u]\n", 1);
#ifdef ZSTD_GZCOMPRESS
    if (exeNameMatch(programName, ZSTD_GZ)) { /* behave like gzip */
    ...
    DISPLAYOUT(
        "  --patch-from=REF              Use REF as the reference point for Zstandard's diff "
        "engine. \n");
    DISPLAYOUT("  --patch-apply                 Equivalent for `-d --patch-from` \n\n");
#ifdef ZSTD_MULTITHREAD
    DISPLAYOUT(
        "  -T#                           Spawn # compression threads. [Default: %u; pass 0 for "
        "core count.]\n",
        init_nbWorkers(ZSTDCLI_NBTHREADS_DEFAULT));
    DISPLAYOUT(
        "  --single-thread               Share a single thread for I/O and compression (slightly "
        "different than `-T1`).\n");
    DISPLAYOUT("  --auto-threads={physical|logical}\n");
    DISPLAYOUT(
        "                                Use physical/logical cores when using `-T0`. [Default: "
        "Physical]\n\n");
    DISPLAYOUT("  --jobsize=#                   Set job size to #. [Default: 0 (automatic)]\n");
    DISPLAYOUT(
        "  --rsyncable                   Compress using a rsync-friendly method (`--jobsize=#` "
        "sets unit size). \n");
    DISPLAYOUT("\n");
#endif
    DISPLAYOUT(
    ...
    DISPLAYOUT(
        "  --size-hint=#                 Optimize compression parameters for streaming input of "
        "approximately size #.\n");
    DISPLAYOUT("  --target-compressed-block-size=#\n");
    DISPLAYOUT(
        "                                Generate compressed blocks of approximately # size.\n\n");
    DISPLAYOUT(
        "  --no-dictID                   Don't write `dictID` into the header (dictionary "
        "compression only).\n");
    DISPLAYOUT("  --[no-]compress-literals      Force (un)compressed literals.\n");
    DISPLAYOUT(
        "  --[no-]row-match-finder       Explicitly enable/disable the fast, row-based matchfinder "
        "for\n");
    ...
    DISPLAYOUT(
        "  --[no-]mmap-dict              Memory-map dictionary file rather than mallocing and "
        "loading all at once\n");
#ifdef ZSTD_GZCOMPRESS
    ...
    DISPLAYOUT(
        "  --[no-]sparse                 Enable sparse mode. [Default: Enabled for files, disabled "
        "for STDOUT.]\n");
#else
...
#endif
    {
        ...
        DISPLAYOUT(
            "  --[no-]pass-through           Pass through uncompressed files as-is. [Default: "
            "%s]\n",
            passThroughDefault);
    }
    ...
    DISPLAYOUT(
        "                                Use the fast cover algorithm (with optional "
        "arguments).\n\n");
    DISPLAYOUT(
        "  --train-legacy[=s=#]          Use the legacy algorithm with selectivity #. [Default: "
        "%u]\n",
    ...
    DISPLAYOUT(
        "  -b#                           Perform benchmarking with compression level #. [Default: "
        "%d]\n",
        ZSTDCLI_CLEVEL_DEFAULT);
    DISPLAYOUT(
        "  -e#                           Test all compression levels up to #; starting level is "
        "`-b#`. [Default: 1]\n");
    DISPLAYOUT(
        "  -i#                           Set the minimum evaluation to time # seconds. [Default: "
        "3]\n");
    DISPLAYOUT(
        "  --split=#                     Split input into independent chunks of size #. [Default: "
        "No chunking]\n");
    DISPLAYOUT(
        "  -S                            Output one benchmark result per input file. [Default: "
        "Consolidated result]\n");
...
static Task<void> waitEnter(void)
{
    DISPLAY("Press enter to continue... \n");
    co_await zstd_flush();
    co_await b_fgetc(stdin);
}
...
/* Upstream exit()ed here, from inside a value-returning parser. The value it
 * would have returned is meaningless once this fires, and the caller is main's
 * argument loop, which checks zstd_fatal() once the loop is over. */
...
static int readU32FromCharChecked(const char **stringPtr, unsigned *value)
{
    ...
    if ((**stringPtr == 'K') || (**stringPtr == 'M') || (**stringPtr == 'G')) {
        switch (**stringPtr) {
        case 'K':
            if (result > (((unsigned)-1) >> 10))
                return 1; /* overflow error */
            result <<= 10;
            break;
        case 'M':
            if (result > (((unsigned)-1) >> 20))
                return 1; /* overflow error */
            result <<= 20;
            break;
        case 'G':
            if (result > (((unsigned)-1) >> 30))
                return 1; /* overflow error */
            result <<= 30;
            break;
        }
...
static int readSizeTFromCharChecked(const char **stringPtr, size_t *value)
{
    ...
    if ((**stringPtr == 'K') || (**stringPtr == 'M') || (**stringPtr == 'G')) {
        switch (**stringPtr) {
        case 'K':
            if (result > (((size_t)-1) >> 10))
                return 1; /* overflow error */
            result <<= 10;
            break;
        case 'M':
            if (result > (((size_t)-1) >> 20))
                return 1; /* overflow error */
            result <<= 20;
            break;
        case 'G':
            if (result > (((size_t)-1) >> 30))
                return 1; /* overflow error */
            result <<= 30;
            break;
        }
...
static unsigned parseCoverParameters(const char *stringPtr, ZDICT_cover_params_t *params)
{
    ...
    DISPLAYLEVEL(4, "cover: k=%u\nd=%u\nsteps=%u\nsplit=%u\nshrink%u\n", params->k, params->d,
                 params->steps, (unsigned)(params->splitPoint * 100),
...
static unsigned parseFastCoverParameters(const char *stringPtr, ZDICT_fastCover_params_t *params)
{
    ...
    if (stringPtr[0] != 0)
        return 0;
    DISPLAYLEVEL(4, "cover: k=%u\nd=%u\nf=%u\nsteps=%u\nsplit=%u\naccel=%u\nshrink=%u\n", params->k,
                 params->d, params->f, params->steps, (unsigned)(params->splitPoint * 100),
                 params->accel, params->shrinkDictMaxRegression);
    return 1;
}

/**
 * parseLegacyParameters() :
 * reads legacy dictionary builder parameters from *stringPtr (e.g. "--train-legacy=selectivity=8")
 * into *selectivity
 ...
 *  reads adapt parameters from *stringPtr (e.g. "--adapt=min=1,max=19) and store them into
 * adaptMinPtr and adaptMaxPtr. Both adaptMinPtr and adaptMaxPtr must be already allocated and
 * correctly initialized. There is no guarantee that any of these values will be updated.
 ...
 *  @return 0 in case of malformed parameters
 */
static unsigned parseCompressionParameters(const char *stringPtr,
...
static Task<void> printDefaultCParams(const char *filename, const char *dictFileName, int cLevel)
{
    unsigned long long fileSize = co_await UTIL_getFileSize(filename);
    const size_t dictSize =
        dictFileName != NULL ? (size_t)co_await UTIL_getFileSize(dictFileName) : 0;
    ...
    DISPLAY(" - strategy      : %s (%u)\n", ZSTD_strategyMap[(int)cParams.strategy],
            (unsigned)cParams.strategy);
}

static Task<void> printActualCParams(const char *filename, const char *dictFileName, int cLevel,
                                     const ZSTD_compressionParameters *cParams)
{
    ...
    actualCParams.windowLog =
        cParams->windowLog == 0 ? actualCParams.windowLog : cParams->windowLog;
    ...
    actualCParams.searchLog =
        cParams->searchLog == 0 ? actualCParams.searchLog : cParams->searchLog;
    actualCParams.minMatch = cParams->minMatch == 0 ? actualCParams.minMatch : cParams->minMatch;
    actualCParams.targetLength =
        cParams->targetLength == 0 ? actualCParams.targetLength : cParams->targetLength;
    ...
    DISPLAY("--zstd=wlog=%d,clog=%d,hlog=%d,slog=%d,mml=%d,tlen=%d,strat=%d\n",
            actualCParams.windowLog, actualCParams.chainLog, actualCParams.hashLog,
            actualCParams.searchLog, actualCParams.minMatch, actualCParams.targetLength,
...
#define ENV_NBWORKERS \
    "ZSTD_NBTHREADS" /* takes lower precedence than directly specifying -T# in the CLI */
    ...
    if (env != NULL) {
        ...
        if ((*ptr >= '0') && (*ptr <= '9')) {
            ...
            if (readU32FromCharChecked(&ptr, &absLevel)) {
                DISPLAYLEVEL(
                    2, "Ignore environment variable setting %s=%s: numeric value too large \n",
                    ENV_CLEVEL, env);
...
#define NEXT_UINT32(_varu32)                                                                    \
    {                                                                                           \
        ...
        if (*__nb != 0) {                                                                       \
            errorOut(                                                                           \
                "error: only numeric values with optional suffixes K, KB, KiB, M, MB, MiB, G, " \
                "GB, GiB are allowed");                                                         \
        }                                                                                       \
    }
...
/* Upstream's main(), whole. zstd_main() below is what proc_main() calls, and it
 * exists only to hand this the argv it was written for. */
static Task<int> zstdcli_main(int argCount, const char *argv[])
{
    int argNb, followLinks = 0, allowBlockDevices = 0, forceStdin = 0, forceStdout = 0,
               hasStdout = 0, ldmFlag = 0, main_pause = 0, adapt = 0, adaptMin = MINCLEVEL,
               adaptMax = MAXCLEVEL, rsyncable = 0, nextArgumentsAreFiles = 0, operationResult = 0,
               separateFiles = 0, setRealTimePrio = 0, singleThread = 0, defaultLogicalCores = 0,
               showDefaultCParams = 0, contentSize = 1, removeSrcFile = 0, cLevel = init_cLevel(),
               ultra = 0, cLevelLast = MINCLEVEL - 1, /* for benchmark range */
        setThreads_non1                  = 0;
    ...
    FileNamesTable *file_of_names =
        UTIL_allocateFileNamesTable((size_t)argCount); /* argCount >= 1 */
    ...
    if (exeNameMatch(programName, ZSTD_ZSTDMT))
        nbWorkers = NBWORKERS_AUTOCPU, singleThread = 0;
        ...
        if (argument[0] == '-') {
            if (argument[1] == '-') {
                ...
                if (exeNameMatch(programName, ZSTD_GZ)) { /* behave like gzip */
                    ...
                    if (!strcmp(argument, "--no-name")) { /* ignore for now */
                        ;
                        continue;
                    }
                }
                ...
                if (longCommandWArg(&argument, "--block-size")) {
                    ...
                    continue;
                } /* hidden command, prefer --split below */
                if (longCommandWArg(&argument, "--split")) {
                    NEXT_TSIZE(chunkSize);
                    continue;
                }
                if (longCommandWArg(&argument, "--jobsize")) {
                    NEXT_TSIZE(chunkSize);
                    continue;
                } /* note: overloaded variable */
                ...
                if (longCommandWArg(&argument, "--output-dir-flat")) {
                    ...
                    if (strlen(outDirName) == 0) {
                        DISPLAYLEVEL(1,
                                     "error: output dir cannot be empty string (did you mean to "
                                     "pass '.' instead?)\n");
                        CLEAN_RETURN(1);
                    }
                    continue;
                ...
                if (longCommandWArg(&argument, "--patch-apply")) {
                    operation = zom_decompress;
                    NEXT_FIELD(patchFromDictFileName);
                    memLimit = 1U << ZSTD_WINDOWLOG_MAX;
                    continue;
                }
    ...
    /* errorOut() recorded rather than exiting, so this is where an overflowed
     * numeric argument stops the run. */
    if (zstd_fatal())
        CLEAN_RETURN(zstd_status());
    ...
    if (!followLinks) {
        ...
        for (u = 0, fileNamesNb = 0; u < nbFilenames; u++) {
            if (co_await UTIL_isLink(filenames->fileNames[u]) &&
                !co_await UTIL_isFIFO(filenames->fileNames[u])) {
    ...
    if (file_of_names->tableSize) {
        ...
        for (flNb = 0; flNb < nbFileLists; flNb++) {
            FileNamesTable *const fnt =
                co_await UTIL_createFileNamesTable_fromFileList(file_of_names->fileNames[flNb]);
            if (fnt == NULL) {
    ...
    if (recursive) { /* at this stage, filenameTable is a list of paths, which can contain both
                        files and directories */
        co_await UTIL_expandFNT(&filenames, followLinks);
    }
#else
    (void)followLinks;
#endif
...
#ifndef ZSTD_NODECOMPRESS
        int const ret = co_await FIO_listMultipleFiles((unsigned)filenames->tableSize,
                                                       filenames->fileNames, g_displayLevel);
        CLEAN_RETURN(ret);
#else
    ...
    if (operation == zom_bench) {
        ...
        if (nbWorkers == NBWORKERS_UNSET)
            nbWorkers = 1;
        if (nbWorkers <= 1)
            benchThreadsMsgLevel = 3;
            ...
            if (separateFiles) {
                ...
                for (i = 0; i < filenames->tableSize; i++) {
                    operationResult = BMK_benchFilesAdvanced(
                        &filenames->fileNames[i], 1, dictFileName, cLevel, cLevelLast,
                        &compressionParams, g_displayLevel, &benchParams);
                }
            } else {
                operationResult = BMK_benchFilesAdvanced(
                    filenames->fileNames, (unsigned)filenames->tableSize, dictFileName, cLevel,
                    cLevelLast, &compressionParams, g_displayLevel, &benchParams);
            }
        } else {
            operationResult = BMK_syntheticTest(compressibility, cLevel, cLevelLast,
                                                &compressionParams, g_displayLevel, &benchParams);
        }
    ...
    if (operation == zom_train) {
        ...
        if (nbWorkers == NBWORKERS_UNSET)
            nbWorkers = init_nbWorkers(ZSTDCLI_NBTHREADS_DEFAULT);
        if (dict == cover) {
            int const optimize    = !coverParams.k || !coverParams.d;
            coverParams.nbThreads = (unsigned)nbWorkers;
            coverParams.zParams   = zParams;
            operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenames->fileNames,
                                                 (int)filenames->tableSize, chunkSize, NULL,
                                                 &coverParams, NULL, optimize, memLimit);
        } else if (dict == fastCover) {
            int const optimize        = !fastCoverParams.k || !fastCoverParams.d;
            fastCoverParams.nbThreads = (unsigned)nbWorkers;
            fastCoverParams.zParams   = zParams;
            operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenames->fileNames,
                                                 (int)filenames->tableSize, chunkSize, NULL, NULL,
                                                 &fastCoverParams, optimize, memLimit);
        } else {
            ...
            operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenames->fileNames,
                                                 (int)filenames->tableSize, chunkSize, &dictParams,
                                                 NULL, NULL, 0, memLimit);
        }
#else
        (void)dictCLevel;
        (void)dictSelect;
    ...
    if (filenames->tableSize == 0) {
        ...
        if (nbInputFileNames > 0) {
            DISPLAYLEVEL(
                1, "please provide correct input file(s) or non-empty directories -- ignored \n");
            CLEAN_RETURN(0);
        }
        UTIL_refFilename(filenames, stdinmark);
    }
    ...
    if (!forceStdin && (UTIL_searchFileNamesTable(filenames, stdinmark) != -1) &&
        co_await UTIL_isConsole(stdin)) {
        DISPLAYLEVEL(1, "stdin is a console, aborting\n");
        CLEAN_RETURN(1);
    }
    if ((!outFileName || !strcmp(outFileName, stdoutmark)) && co_await UTIL_isConsole(stdout) &&
        (UTIL_searchFileNamesTable(filenames, stdinmark) != -1) && !forceStdout &&
        operation != zom_decompress) {
    ...
    /* check compression level limits */
    {
        ...
        if (cLevel > maxCLevel) {
            DISPLAYLEVEL(2, "Warning : compression level higher than max, reduced to %i. ",
                         maxCLevel);
            DISPLAYLEVEL(2,
                         "Specify --ultra to raise the limit to 22 and use "
    ...
    if (!co_await UTIL_isConsole(stderr) && (progress != FIO_ps_always))
        progress = FIO_ps_never;
    ...
    if (operation == zom_compress) {
        ...
        if (showDefaultCParams || g_displayLevel >= 4) {
            ...
            for (fileNb = 0; fileNb < (size_t)filenames->tableSize; fileNb++) {
                if (showDefaultCParams)
                    co_await printDefaultCParams(filenames->fileNames[fileNb], dictFileName,
                                                 cLevel);
                if (g_displayLevel >= 4)
                    co_await printActualCParams(filenames->fileNames[fileNb], dictFileName, cLevel,
                                                &compressionParams);
            }
        }
        ...
        if ((filenames->tableSize == 1) && outFileName)
            operationResult =
                co_await FIO_compressFilename(fCtx, prefs, outFileName, filenames->fileNames[0],
                                              dictFileName, cLevel, compressionParams);
        else
            operationResult = co_await FIO_compressMultipleFilenames(
                fCtx, prefs, filenames->fileNames, outMirroredDirName, outDirName, outFileName,
...
#ifndef ZSTD_NODECOMPRESS
        if (filenames->tableSize == 1 && outFileName) {
            operationResult = co_await FIO_decompressFilename(
                fCtx, prefs, outFileName, filenames->fileNames[0], dictFileName);
        } else {
            operationResult = co_await FIO_decompressMultipleFilenames(
                fCtx, prefs, filenames->fileNames, outMirroredDirName, outDirName, outFileName,
                dictFileName);
        }
#else
...
/* Braam's entry point. Upstream's parser wants a NUL-terminated argv and Args
 * is a list of Str, so the vector is built here and freed at the end; argv[0]
 * is what the exeNameMatch() presets read, so the link a user typed decides
 * which program this behaves as. */
Task<i32> zstd_main(Args args)
{
    int const argCount = (int)args.size();
    const char **argv  = (const char **)calloc((size_t)argCount + 1, sizeof(char *));
    int argNb, result;
    ...
    for (argNb = 0; argNb < argCount; argNb++) {
        size_t const len = args[(usize)argNb].size();
        char *const s    = (char *)malloc(len + 1);
        if (s == NULL) {
            DISPLAYLEVEL(1, "zstd: allocation error \n");
            co_return 1;
        }
        memcpy(s, args[(usize)argNb].data(), len);
        s[len]      = 0;
        argv[argNb] = s;
    }

    result = co_await zstdcli_main(argCount, argv);

    for (argNb = 0; argNb < argCount; argNb++)
        free((void *)argv[argNb]);
    free(argv);
    co_return result;
}
