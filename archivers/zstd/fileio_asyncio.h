 ...
 *
 * Braam: there are no threads, so this is upstream's synchronous path alone --
 * the one it takes when ZSTD_MULTITHREAD is not defined. The mutex, the
 * condition variable and the thread pool are gone with it; two jobs remain, as
 * upstream keeps in that build, and the pool function is a Task because the
 * read and the write it performs are.
 */
...
typedef struct {
    ...
    /* Coalesce buffer is used to join two buffers in case where we need to read more bytes than
     * left in the first of them. Shouldn't be accessed from outside ot utility functions. */
    U8 *coalesceBuffer;

    /* Read buffer can be used by consumer code, take care when copying this pointer aside as it
     * might change when consuming / refilling buffer. */
    U8 *srcBuffer;
...
typedef struct {
    ...
    /* This field should be changed before a job is queued for execution and should contain the
 ...
 * Allows (de)activating async mode. Nothing is asynchronous here. */
 ...
 * bufferSize). Returns if srcBuffer has at least n bytes loaded or if we've reached the end of the
 * file. Return value is the number of bytes added to the buffer. Note that srcBuffer might have up
 * to 2 times bufferSize bytes. */
