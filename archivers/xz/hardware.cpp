...
extern void hardware_memlimit_set(uint64_t new_memlimit, bool set_compress, bool set_decompress,
                                  bool set_mtdec, bool is_percentage)
{
    if (is_percentage) {
    ...
    if (set_compress) {
        ...
        if (memlimit_compress != UINT64_MAX && memlimit_compress > limit_max)
...
extern uint64_t hardware_memlimit_get(enum operation_mode mode)
{
    ...
    const uint64_t memlimit = mode == MODE_COMPRESS ? memlimit_compress : memlimit_decompress;
    return memlimit != 0 ? memlimit : UINT64_MAX;
}

extern uint64_t hardware_memlimit_mtenc_get(void)
{
    return hardware_memlimit_mtenc_is_default() ? memlimit_mt_default
...
{
    uint64_t m = memlimit_mtdec != 0 ? memlimit_mtdec : memlimit_mt_default;
...
static void memlimit_show(const char *str, size_t str_columns, uint64_t value)
{
    ...
    if (value == 0 || value == UINT64_MAX)
        message(V_VERBOSE, "  %s: %s", str, _("Disabled"));
    else
        message(V_VERBOSE, "  %s: %s MiB (%s B)", str, uint64_to_str(round_up_to_mib(value), 0),
...
extern void hardware_memlimit_show(void)
{
    ...
    if (opt_robot) {
        message(V_VERBOSE,
                "%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu32,
                total_ram, memlimit_compress, memlimit_decompress, hardware_memlimit_mtdec_get(),
                memlimit_mt_default, cputhreads);
    } else {
        message(V_VERBOSE, "%s", _("Hardware information:"));
        memlimit_show(_("Amount of physical memory (RAM):"), 0, total_ram);
        message(V_VERBOSE, "  %s: %" PRIu32, _("Number of processor threads:"), cputhreads);
        message(V_VERBOSE, "%s", _("Memory usage limits:"));
        memlimit_show(_("Compression:"), 0, memlimit_compress);
        memlimit_show(_("Decompression:"), 0, memlimit_decompress);
        memlimit_show(_("Multi-threaded decompression:"), 0, hardware_memlimit_mtdec_get());
        memlimit_show(_("Default for -T0:"), 0, memlimit_mt_default);
    }
...
extern void hardware_init(void)
{
    ...
    if (total_ram == 0)
        total_ram = XZ_PROC_RAM;

    memlimit_mt_default = total_ram / 4;

    if (memlimit_compress == 0)
        memlimit_compress = XZ_PROC_RAM - XZ_COMPRESS_HEADROOM;

#if 0 // HAVE_GETRLIMIT — not used on Braam
