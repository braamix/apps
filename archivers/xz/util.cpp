...
#if defined(__DJGPP__) || defined(_MSC_VER) || (defined(__MINGW32__) && __USE_MINGW_ANSI_STDIO == 0)
#define FORMAT_THOUSAND_SEP(prefix, suffix) prefix suffix
#define check_thousand_sep(slot) \
    do {                         \
    } while (0)
#else
#define FORMAT_THOUSAND_SEP(prefix, suffix) \
    ((thousand == WORKS) ? prefix "'" suffix : prefix suffix)
...
extern uint64_t str_to_uint64(const char *name, const char *value, uint64_t min, uint64_t max)
{
    ...
    if (*value != '\0') {
        ...
        if (*value != '\0' && strcmp(value, "i") != 0 && strcmp(value, "iB") != 0 &&
            strcmp(value, "B") != 0)
            multiplier = 0;

        if (multiplier == 0) {
            message(V_ERROR, _("%s: Invalid multiplier suffix"), tuklib_mask_nonprint(value - 1));
            message_fatal(
...
extern const char *uint64_to_str(uint64_t value, uint32_t slot)
{
    ...
    snprintf(bufs[slot], sizeof(bufs[slot]), FORMAT_THOUSAND_SEP("%", PRIu64), value);
                                     ...
                                     uint32_t slot)
{
    ...
    if ((unit_min == NICESTR_B && value < 10000) || unit_max == NICESTR_B) {
        ...
        my_snprintf(&pos, &left, FORMAT_THOUSAND_SEP("%", "u"), (unsigned int)value);
    } else {
        ...
        do {
            d /= 1024.0;
            unit = (enum nicestr_unit)((int)unit + 1);
    ...
    if (always_also_bytes && value >= 10000)
        snprintf(pos, left, FORMAT_THOUSAND_SEP(" (%", PRIu64 " B)"), value);
...
extern bool is_tty_stdin(void)
{
    ...
    if (ret)
        message_error(
            _("Compressed data cannot be read from "
              "a terminal"));
...
extern bool is_tty_stdout(void)
{
    ...
    if (ret)
        message_error(
            _("Compressed data cannot be written to "
              "a terminal"));
