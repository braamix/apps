...
static void parse_options(const char *str, const option_map *opts,
                          void (*set)(void *filter_options, unsigned key, uint64_t value,
                                      const char *valuestr),
                          void *filter_options)
{
    if (str == NULL || str[0] == '\0')
    ...
    while (*name != '\0') {
        ...
        while (true) {
            if (opts[i].name == NULL)
                message_fatal(_("%s: Invalid option name"), tuklib_mask_nonprint(name));
        ...
        if (opts[i].map != NULL) {
            ...
            if (opts[i].map[j].name == NULL)
                message_fatal(_("%s: %s"), tuklib_mask_nonprint(value), _("Invalid option value"));
            ...
            // value is an integer.
            const uint64_t v = str_to_uint64(name, value, opts[i].min, opts[i].max);
...
{
    static const option_map opts[] = { { "dist", NULL, LZMA_DELTA_DIST_MIN, LZMA_DELTA_DIST_MAX },
                                       { NULL, NULL, 0, 0 } };
...
{
    static const option_map opts[] = { { "start", NULL, 0, UINT32_MAX }, { NULL, NULL, 0, 0 } };
...
tuklib_attr_noreturn static void error_lzma_preset(const char *valuestr)
{
    message_fatal(_("Unsupported LZMA1/LZMA2 preset: %s"), tuklib_mask_nonprint(valuestr));
}
...
static void set_lzma(void *options, unsigned key, uint64_t value, const char *valuestr)
{
    ...
    case OPT_MF:
        opt->mf = (lzma_match_finder)value;
        break;
...
{
    static const name_id_map modes[] = { { "fast", LZMA_MODE_FAST },
                                         { "normal", LZMA_MODE_NORMAL },
                                         { NULL, 0 } };

    static const name_id_map mfs[] = { { "hc3", LZMA_MF_HC3 }, { "hc4", LZMA_MF_HC4 },
                                       { "bt2", LZMA_MF_BT2 }, { "bt3", LZMA_MF_BT3 },
                                       { "bt4", LZMA_MF_BT4 }, { NULL, 0 } };

    static const option_map opts[] = { { "preset", NULL, UINT64_MAX, 0 },
