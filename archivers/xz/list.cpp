...
static struct {
    ...
    /// Field width to use for xz_out_print() to pad "str" to use "columns"
    /// number of columns on a terminal. This is calculated in
    /// init_headings().
    int fw;

} headings[] = {
    { N_("Stream"), 6, 0 },        { N_("Block"), 9, 0 },
    { N_("Blocks"), 9, 0 },        { N_("CompOffset"), 15, 0 },
    { N_("UncompOffset"), 15, 0 }, { N_("CompSize"), 15, 0 },
    { N_("UncompSize"), 15, 0 },   { N_("TotalSize"), 15, 0 },
    { N_("Ratio"), 5, 0 },         { N_("Check"), 10, 0 },
    { N_("CheckVal"), 1, 0 },      { N_("Padding"), 7, 0 },
    { N_("Header"), 5, 0 },        { N_("Flags"), 2, 0 },
...
static void init_colon_strs(void)
{
    ...
    // Calculate the field width for xz_out_print("%*s") so that the strings
...
static void init_headings(void)
{
    ...
    for (unsigned i = 0; i < ARRAY_SIZE(headings); ++i) {
        ...
        // Calculate the field width for xz_out_print("%*s") so that
...
///
static Task<bool> parse_indexes(xz_file_info *xfi, file_pair *pair)
{
    if (pair->src_st.st_size <= 0) {
        message_error(_("%s: File is empty"), tuklib_mask_nonprint(pair->src_name));
        co_return true;
    }
    ...
    lzma_ret ret = lzma_file_info_decoder(&strm, &idx, hardware_memlimit_get(MODE_LIST),
                                          (uint64_t)(pair->src_st.st_size));
    if (ret != LZMA_OK) {
        message_error(_("%s: %s"), tuklib_mask_nonprint(pair->src_name), message_strm(ret));
        co_return true;
    }
    ...
    while (!failed) {
        if (strm.avail_in == 0) {
            strm.next_in  = list_buf.u8;
            strm.avail_in = co_await io_read(pair, &list_buf, IO_BUFFER_SIZE);
        ...
        if (ret == LZMA_SEEK_NEEDED) {
            assert(strm.seek_pos <= (uint64_t)(pair->src_st.st_size));
            if (co_await io_seek_src(pair, strm.seek_pos))
                failed = true;
            else
                strm.avail_in = 0;
            continue;
...
static Task<bool> parse_block_header(file_pair *pair, const lzma_index_iter *iter,
...
#error IO_BUFFER_SIZE < LZMA_BLOCK_HEADER_SIZE_MAX
    ...
    const uint32_t size =
        my_min(iter->block.total_size - lzma_check_size(iter->stream.flags->check),
               LZMA_BLOCK_HEADER_SIZE_MAX);
    if (co_await io_pread(pair, &list_buf, size, iter->block.compressed_file_offset))
        co_return true;
    ...
    block.header_size = lzma_block_header_size_decode(list_buf.u8[0]);
    ...
    switch (lzma_block_header_decode(&block, NULL, list_buf.u8)) {
    case LZMA_OK:
        break;
    ...
    if (xfi->min_version < 50000022U) {
        ...
        if (filters[i].id == LZMA_FILTER_LZMA2 && iter->block.uncompressed_size == 0)
            xfi->min_version = 50000022U;
    }

    // Convert the filter chain to human readable form.
    const lzma_ret str_ret = lzma_str_from_filters(&bhi->filter_chain, filters,
    ...
    if (str_ret != LZMA_OK) {
        message_error(_("%s: %s"), tuklib_mask_nonprint(pair->src_name), message_strm(str_ret));
        co_return true;
    }
...
///
/// \return     False on success, true on error.
static Task<bool> parse_check_value(file_pair *pair, const lzma_index_iter *iter)
{
    ...
    const uint64_t offset = iter->block.compressed_file_offset + iter->block.total_size - size;
    if (co_await io_pread(pair, &list_buf, size, offset))
        co_return true;

    if (size == 4)
        snprintf(check_value, sizeof(check_value), "%08" PRIx32, conv32le(list_buf.u32[0]));
    else if (size == 8)
        snprintf(check_value, sizeof(check_value), "%016" PRIx64, conv64le(list_buf.u64[0]));
    else
        for (size_t i = 0; i < size; ++i)
            snprintf(check_value + i * 2, 3, "%02x", list_buf.u8[i]);
...
static Task<bool> parse_details(file_pair *pair, const lzma_index_iter *iter,
                                block_header_info *bhi, xz_file_info *xfi)
{
    if (co_await parse_block_header(pair, iter, bhi, xfi))
        co_return true;

    if (co_await parse_check_value(pair, iter))
        co_return true;
...
static const char *get_ratio(uint64_t compressed_size, uint64_t uncompressed_size)
{
    ...
    const double ratio = (double)(compressed_size) / (double)(uncompressed_size);
    if (ratio > 9.999)
        return "---";
...
static bool print_info_basic(const xz_file_info *xfi, file_pair *pair)
{
    ...
    if (!headings_displayed) {
        ...
        xz_out_print("%s\n", _("Strms  Blocks   Compressed Uncompressed  Ratio  "
                               "Check   Filename"));
    }
    ...
    const char *cols[6] = {
        ...
        uint64_to_nicestr(lzma_index_file_size(xfi->idx), NICESTR_B, NICESTR_TIB, false, 2),
        uint64_to_nicestr(lzma_index_uncompressed_size(xfi->idx), NICESTR_B, NICESTR_TIB, false, 3),
        get_ratio(lzma_index_file_size(xfi->idx), lzma_index_uncompressed_size(xfi->idx)),
        checks,
    };
    xz_out_print("%*s %*s  %*s  %*s  %*s  %-*s %s\n", tuklib_mbstr_fw(cols[0], 5), cols[0],
                 tuklib_mbstr_fw(cols[1], 7), cols[1], tuklib_mbstr_fw(cols[2], 11), cols[2],
                 tuklib_mbstr_fw(cols[3], 11), cols[3], tuklib_mbstr_fw(cols[4], 5), cols[4],
                 tuklib_mbstr_fw(cols[5], 7), cols[5], tuklib_mask_nonprint(pair->src_name));

    return false;
}

static void print_adv_helper(uint64_t stream_count, uint64_t block_count, uint64_t compressed_size,
                             uint64_t uncompressed_size, uint32_t checks, uint64_t stream_padding)
{
    char checks_str[CHECKS_STR_SIZE];
    get_check_names(checks_str, checks, true);

    xz_out_print("  %-*s %s\n", COLON_STR(COLON_STR_STREAMS), uint64_to_str(stream_count, 0));
    xz_out_print("  %-*s %s\n", COLON_STR(COLON_STR_BLOCKS), uint64_to_str(block_count, 0));
...
static Task<bool> print_info_adv(xz_file_info *xfi, file_pair *pair)
{
    // Print the overall information.
    print_adv_helper(lzma_index_stream_count(xfi->idx), lzma_index_block_count(xfi->idx),
    ...
    xz_out_print("  %s\n    %*s %*s %*s %*s %*s %*s  %*s  %-*s %*s\n",
                 _(colon_strs[COLON_STR_STREAMS]), HEADING_STR(HEADING_STREAM),
                 HEADING_STR(HEADING_BLOCKS), HEADING_STR(HEADING_COMPOFFSET),
                 HEADING_STR(HEADING_UNCOMPOFFSET), HEADING_STR(HEADING_COMPSIZE),
                 HEADING_STR(HEADING_UNCOMPSIZE), HEADING_STR(HEADING_RATIO),
                 HEADING_STR(HEADING_CHECK), HEADING_STR(HEADING_PADDING));
    ...
    while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_STREAM)) {
        ...
        xz_out_print("    %*s %*s %*s %*s ",
                     tuklib_mbstr_fw(cols1[0], headings[HEADING_STREAM].columns), cols1[0],
                     tuklib_mbstr_fw(cols1[1], headings[HEADING_BLOCKS].columns), cols1[1],
                     tuklib_mbstr_fw(cols1[2], headings[HEADING_COMPOFFSET].columns), cols1[2],
                     tuklib_mbstr_fw(cols1[3], headings[HEADING_UNCOMPOFFSET].columns), cols1[3]);
        ...
        xz_out_print("%*s %*s  %*s  %-*s %*s\n",
                     tuklib_mbstr_fw(cols2[0], headings[HEADING_COMPSIZE].columns), cols2[0],
                     tuklib_mbstr_fw(cols2[1], headings[HEADING_UNCOMPSIZE].columns), cols2[1],
                     tuklib_mbstr_fw(cols2[2], headings[HEADING_RATIO].columns), cols2[2],
                     tuklib_mbstr_fw(cols2[3], headings[HEADING_CHECK].columns), cols2[3],
                     tuklib_mbstr_fw(cols2[4], headings[HEADING_PADDING].columns), cols2[4]);
    ...
    if (lzma_index_block_count(xfi->idx) > 0) {
        ...
        // used as is as the field width for xz_out_print() when printing
        ...
        const int checkval_width = my_max(headings[HEADING_CHECKVAL].columns, (int)(2 * check_max));

        // All except Check are right aligned; Check is left aligned.
        xz_out_print("  %s\n    %*s %*s %*s %*s %*s %*s  %*s  %-*s",
                     _(colon_strs[COLON_STR_BLOCKS]), HEADING_STR(HEADING_STREAM),
                     HEADING_STR(HEADING_BLOCK), HEADING_STR(HEADING_COMPOFFSET),
                     HEADING_STR(HEADING_UNCOMPOFFSET), HEADING_STR(HEADING_TOTALSIZE),
                     HEADING_STR(HEADING_UNCOMPSIZE), HEADING_STR(HEADING_RATIO),
                     detailed ? headings[HEADING_CHECK].fw : 1, _(headings[HEADING_CHECK].str));
        ...
        if (detailed) {
            ...
            xz_out_print(
                " %-*s  %*s  %-*s %*s %*s  %s",
                headings[HEADING_CHECKVAL].fw + checkval_width - headings[HEADING_CHECKVAL].columns,
                _(headings[HEADING_CHECKVAL].str), HEADING_STR(HEADING_HEADERSIZE),
                HEADING_STR(HEADING_HEADERFLAGS), HEADING_STR(HEADING_COMPSIZE),
                HEADING_STR(HEADING_MEMUSAGE), _(headings[HEADING_FILTERS].str));
        }
        ...
        while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_BLOCK)) {
            ...
            if (detailed && (co_await parse_details(pair, &iter, &bhi, xfi)))
                co_return true;

            const char *cols1[4] = { uint64_to_str(iter.stream.number, 0),
            ...
            xz_out_print(
                "    %*s %*s %*s %*s ", tuklib_mbstr_fw(cols1[0], headings[HEADING_STREAM].columns),
                cols1[0], tuklib_mbstr_fw(cols1[1], headings[HEADING_BLOCK].columns), cols1[1],
                tuklib_mbstr_fw(cols1[2], headings[HEADING_COMPOFFSET].columns), cols1[2],
                tuklib_mbstr_fw(cols1[3], headings[HEADING_UNCOMPOFFSET].columns), cols1[3]);

            const char *cols2[4] = { uint64_to_str(iter.block.total_size, 0),
                                     uint64_to_str(iter.block.uncompressed_size, 1),
                                     get_ratio(iter.block.total_size, iter.block.uncompressed_size),
                                     _(check_names[iter.stream.flags->check]) };
            xz_out_print("%*s %*s  %*s  %-*s",
                         tuklib_mbstr_fw(cols2[0], headings[HEADING_TOTALSIZE].columns), cols2[0],
                         tuklib_mbstr_fw(cols2[1], headings[HEADING_UNCOMPSIZE].columns), cols2[1],
                         tuklib_mbstr_fw(cols2[2], headings[HEADING_RATIO].columns), cols2[2],
                         tuklib_mbstr_fw(cols2[3], detailed ? headings[HEADING_CHECK].columns : 1),
                         cols2[3]);

            if (detailed) {
                const lzma_vli compressed_size = iter.block.unpadded_size - bhi.header_size -
                                                 lzma_check_size(iter.stream.flags->check);

                const char *cols3[6] = { check_value,
                ...
                xz_out_print(
                    " %-*s  %*s  %-*s %*s %*s MiB  %s", checkval_width, cols3[0],
                    tuklib_mbstr_fw(cols3[1], headings[HEADING_HEADERSIZE].columns), cols3[1],
                    tuklib_mbstr_fw(cols3[2], headings[HEADING_HEADERFLAGS].columns), cols3[2],
                    tuklib_mbstr_fw(cols3[3], headings[HEADING_COMPSIZE].columns), cols3[3],
                    tuklib_mbstr_fw(cols3[4], headings[HEADING_MEMUSAGE].columns - 4), cols3[4],
                    cols3[5]);
            }
    ...
    if (detailed) {
        ...
        xz_out_print("  %s %s\n", _("Minimum XZ Utils version:"), xz_ver_to_str(xfi->min_version));
    }

    co_return false;
}

static Task<bool> print_info_robot(xz_file_info *xfi, file_pair *pair)
{
    char checks[CHECKS_STR_SIZE];
    get_check_names(checks, lzma_index_checks(xfi->idx), false);
    ...
    xz_out_print("file\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%s\t%s\t%" PRIu64 "\n",
    ...
    if (message_verbosity_get() >= V_VERBOSE) {
        ...
        while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_STREAM))
            xz_out_print("stream\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64
                         "\t%" PRIu64 "\t%s\t%s\t%" PRIu64 "\n",
                         iter.stream.number, iter.stream.block_count, iter.stream.compressed_offset,
        ...
        while (!lzma_index_iter_next(&iter, LZMA_INDEX_ITER_BLOCK)) {
            ...
            if (message_verbosity_get() >= V_DEBUG &&
                (co_await parse_details(pair, &iter, &bhi, xfi)))
                co_return true;

            xz_out_print("block\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64
                         "\t%" PRIu64 "\t%" PRIu64 "\t%s\t%s",
                         iter.stream.number, iter.block.number_in_stream, iter.block.number_in_file,
                         iter.block.compressed_file_offset, iter.block.uncompressed_file_offset,
                         iter.block.total_size, iter.block.uncompressed_size,
                         get_ratio(iter.block.total_size, iter.block.uncompressed_size),
                         check_names[iter.stream.flags->check]);

            if (message_verbosity_get() >= V_DEBUG)
                xz_out_print("\t%s\t%" PRIu32 "\t%s\t%" PRIu64 "\t%" PRIu64 "\t%s", check_value,
                             bhi.header_size, bhi.flags, bhi.compressed_size, bhi.memusage,
                             bhi.filter_chain);
    ...
    if (message_verbosity_get() >= V_DEBUG)
        xz_out_print("summary\t%" PRIu64 "\t%s\t%" PRIu32 "\n", xfi->memusage_max,
...
static void update_totals(const xz_file_info *xfi)
{
    ...
    const uint64_t uncompressed_size = lzma_index_uncompressed_size(xfi->idx);
    ...
    if (totals.files == UINT64_MAX || UINT64_MAX - totals.streams < streams ||
        UINT64_MAX - totals.blocks < blocks ||
        UINT64_MAX - totals.compressed_size < compressed_size ||
        UINT64_MAX - totals.uncompressed_size < uncompressed_size ||
        UINT64_MAX - totals.stream_padding < xfi->stream_padding) {
        message_fatal(
            _("Error: The total compressed or uncompressed "
...
static void print_totals_basic(void)
{
    ...
    // special handling.
    xz_out_print("%5s %7s  %11s  %11s  %5s  %-7s ", uint64_to_str(totals.streams, 0),
                 uint64_to_str(totals.blocks, 1),
                 uint64_to_nicestr(totals.compressed_size, NICESTR_B, NICESTR_TIB, false, 2),
                 uint64_to_nicestr(totals.uncompressed_size, NICESTR_B, NICESTR_TIB, false, 3),
...
#pragma GCC diagnostic ignored "-Wformat-nonliteral"
    ...
    xz_out_print(
        ngettext("%s file\n", "%s files\n",
                 totals.files <= ULONG_MAX ? totals.files : (totals.files % 1000000) + 1000000),
...
static void print_totals_adv(void)
{
    ...
    print_adv_helper(totals.streams, totals.blocks, totals.compressed_size,
                     totals.uncompressed_size, totals.checks, totals.stream_padding);
...
static void print_totals_robot(void)
{
    ...
    xz_out_print("totals\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%" PRIu64 "\t%s\t%s\t%" PRIu64
                 "\t%" PRIu64,
                 totals.streams, totals.blocks, totals.compressed_size, totals.uncompressed_size,
                 get_ratio(totals.compressed_size, totals.uncompressed_size), checks,
                 totals.stream_padding, totals.files);

    if (message_verbosity_get() >= V_DEBUG)
        xz_out_print("\t%" PRIu64 "\t%s\t%" PRIu32, totals.memusage_max,
...
extern Task<void> list_file(const char *filename)
{
    if (opt_format != FORMAT_XZ && opt_format != FORMAT_AUTO) {
        ...
        if (opt_format == FORMAT_LZMA)
            message(V_ERROR, _("Try 'lzmainfo' with .lzma files."));
    ...
    if (filename == stdin_filename) {
        message_error(
            _("--list does not support reading from "
              "standard input"));
        co_return;
    }
    ...
    opt_stdout      = false;
    opt_force       = true;
    file_pair *pair = co_await io_open_src(filename);
    if (pair == NULL)
        co_return;

    xz_file_info xfi = XZ_FILE_INFO_INIT;
    if (!(co_await parse_indexes(&xfi, pair))) {
        bool fail;

        if (opt_robot)
            fail = co_await print_info_robot(&xfi, pair);
        else if (message_verbosity_get() <= V_WARNING)
            fail = print_info_basic(&xfi, pair);
        else
            fail = co_await print_info_adv(&xfi, pair);
