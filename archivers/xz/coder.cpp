...
static void str_to_filters(const char *str, uint32_t index, uint32_t flags)
{
    int error_pos;
    const char *err = lzma_str_to_filters(str, &error_pos, chains[index], flags, NULL);
    ...
    if (err != NULL) {
        ...
        message(V_ERROR, _("Error in --filters%s=FILTERS option:"), filter_num);
...
extern void coder_add_filters_from_str(const char *filter_str)
{
    ...
    // the string.
    for (filters_count = 0; chains[0][filters_count].id != LZMA_VLI_UNKNOWN; ++filters_count)
        ;
...
tuklib_attr_noreturn static void memlimit_too_small(uint64_t memory_usage)
{
...
static uint64_t get_chains_memusage(uint64_t *chains_memusages, const lzma_mt *mt, bool encode)
{
...
extern void coder_set_compression_settings(void)
{
    ...
    if (opt_block_list != NULL) {
        ...
        const uint32_t missing_chains_mask =
            (block_list_chain_mask ^ chains_used_mask) & block_list_chain_mask;
        ...
        if (missing_chains_mask != 0) {
            ...
            const unsigned first_missing = (unsigned)ctz32(missing_chains_mask);
    ...
    if (filters_count == 0 && chains_used_mask & 1) {
        ...
        default_filters[0].id = opt_format == FORMAT_LZMA ? LZMA_FILTER_LZMA1 : LZMA_FILTER_LZMA2;
    ...
    if (opt_format == FORMAT_LZMA &&
        (filters_count != 1 || default_filters[0].id != LZMA_FILTER_LZMA1))
        message_fatal(
            _("The .lzma format supports only "
              "the LZMA1 filter"));
        ...
        if (opt_format == FORMAT_XZ && hardware_threads_is_mt()) {
            ...
            if (block_size == 0) {
                for (unsigned i = 0; i < ARRAY_SIZE(chains); i++) {
                    ...
                    uint64_t size = lzma_mt_block_size(chains[i]);
                ...
                if (block_list_largest > 0 && block_size > block_list_largest)
            ...
            memory_usage = get_chains_memusage(encoder_memusages, &mt_options, true);
            if (memory_usage != UINT64_MAX)
                message(V_DEBUG, _("Using up to %" PRIu32 " threads."), mt_options.threads);
        } else
#endif
        {
            memory_usage = get_chains_memusage(encoder_memusages, NULL, true);
        }
#endif
    } else {
#ifdef HAVE_DECODERS
    ...
    for (unsigned i = 0; i < ARRAY_SIZE(chains); i++) {
        ...
        unsigned j = 0;
        while (chains[i][j].id != LZMA_FILTER_LZMA2 && chains[i][j].id != LZMA_FILTER_LZMA1) {
        ...
        lzma_options_lzma *opt        = (lzma_options_lzma *)chains[i][j].options;
        ...
        while (true) {
            ...
            encoder_memusages[i] = lzma_raw_encoder_memusage(chains[i]);
        ...
        const char lzma_num    = chains[i][j].id == LZMA_FILTER_LZMA2 ? '2' : '1';
        const char *from_size  = uint64_to_str(orig_dict_size >> 20, 0);
        const char *to_size    = uint64_to_str(opt->dict_size >> 20, 1);
        const char *limit_size = uint64_to_str(round_up_to_mib(memory_limit), 2);
        if (i == 0)
            message(V_WARNING,
                    _("Adjusted LZMA%c dictionary "
...
static bool is_format_lzip(void)
{
...
static enum coder_init_ret coder_init(file_pair *pair)
{
    ...
    lzma_filter *active_filters =
        opt_block_list == NULL ? chains[0] : chains[opt_block_list[0].chain_num];
    ...
    if (opt_mode == MODE_COMPRESS) {
...
#endif
                ret = lzma_stream_encoder(&strm, active_filters, check);
            break;

        case FORMAT_LZMA:
            ret = lzma_alone_encoder(&strm, (lzma_options_lzma *)active_filters[0].options);
            break;
        ...
        case FORMAT_XZ:
            ...
            mt_options.memlimit_threading =
                mt_options.threads == 1 ? 0 : hardware_memlimit_mtdec_get();

            ret = lzma_stream_decoder_mt(&strm, &mt_options);
#else
            ret = lzma_stream_decoder(&strm, hardware_memlimit_get(MODE_DECOMPRESS), flags);
#endif
            break;

        case FORMAT_LZMA:
            ret = lzma_alone_decoder(&strm, hardware_memlimit_get(MODE_DECOMPRESS));
            break;

#ifdef HAVE_LZIP_DECODER
        case FORMAT_LZIP:
            allow_trailing_input = true;
            ret = lzma_lzip_decoder(&strm, hardware_memlimit_get(MODE_DECOMPRESS), flags);
            break;
#endif
        ...
        if (ret == LZMA_OK && init_format != FORMAT_RAW) {
            ...
            while ((ret = lzma_code(&strm, LZMA_RUN)) == LZMA_UNSUPPORTED_CHECK)
                message_warning(_("%s: %s"), tuklib_mask_nonprint(pair->src_name),
                                message_strm(ret));
    ...
    if (*next_block_remaining > 0) {
        ...
        *next_block_remaining -= *block_remaining;
        ...
        if (opt_block_list[*list_pos + 1].size != 0) {
            ...
            if (opt_block_list[*list_pos - 1].chain_num != opt_block_list[*list_pos].chain_num) {
                const unsigned chain_num = opt_block_list[*list_pos].chain_num;
                const lzma_filter *next  = chains[chain_num];
                const lzma_ret ret       = lzma_filters_update(&strm, next);
                ...
                if (ret != LZMA_OK) {
                    ...
                    message_fatal(_("Error changing to "
                                    "filter chain %u: %s"),
...
static Task<bool> coder_write_output(file_pair *pair)
{
    if (opt_mode != MODE_TEST) {
        if (co_await io_write(pair, &out_buf, IO_BUFFER_SIZE - strm.avail_out))
            co_return true;
    }
...
static Task<bool> coder_normal(file_pair *pair)
{
    ...
    if (opt_mode == MODE_COMPRESS && opt_format == FORMAT_XZ) {
        ...
        if (opt_block_list != NULL) {
            if (block_remaining < opt_block_list[list_pos].size) {
                assert(!hardware_threads_is_mt());
                next_block_remaining = opt_block_list[list_pos].size - block_remaining;
            } else {
                block_remaining = opt_block_list[list_pos].size;
            }
        }
    ...
    while (!user_abort) {
...
#ifdef HAVE_ENCODERS
            const size_t read_size = my_min(block_remaining, IO_BUFFER_SIZE);
#else
            const size_t read_size = IO_BUFFER_SIZE;
#endif
            strm.avail_in = co_await io_read(pair, &in_buf, read_size);
        ...
        if (strm.avail_out == 0) {
            if (co_await coder_write_output(pair))
                break;
        }
        ...
        if (ret == LZMA_STREAM_END && (action == LZMA_SYNC_FLUSH || action == LZMA_FULL_BARRIER)) {
            ...
            if (ret == LZMA_STREAM_END) {
                if (allow_trailing_input) {
                    co_await io_fix_src_pos(pair, strm.avail_in);
                    success = true;
                    break;
                }
                ...
                if (strm.avail_in == 0 && !pair->src_eof) {
                    ...
                    // becomes true.
                    strm.avail_in = co_await io_read(pair, &in_buf, 1);
                    if (strm.avail_in == SIZE_MAX)
                        break;
...
static Task<bool> coder_passthru(file_pair *pair)
{
    while (strm.avail_in != 0) {
        if (user_abort)
            co_return false;

        if (co_await io_write(pair, &in_buf, strm.avail_in))
            co_return false;
        ...
        strm.avail_in = co_await io_read(pair, &in_buf, IO_BUFFER_SIZE);
        if (strm.avail_in == SIZE_MAX)
            co_return false;
    }
...
extern Task<void> coder_run(const char *filename)
{
    ...
    if (strm.avail_in != SIZE_MAX) {
        ...
        if (init_ret != CODER_INIT_ERROR && !user_abort) {
            ...
            // is used.
            if (opt_mode == MODE_TEST || !(co_await io_open_dest(pair))) {
                ...
                const uint64_t in_size =
                    pair->src_st.st_size <= 0 ? 0 : (uint64_t)(pair->src_st.st_size);
                message_progress_start(&strm, is_passthru, in_size);

                // Do the actual coding or passthru.
                if (is_passthru)
                    success = co_await coder_passthru(pair);
                else
                    success = co_await coder_normal(pair);
    ...
    co_await io_close(pair, success);
