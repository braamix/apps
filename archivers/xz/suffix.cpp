...
{
    if (len >= 4 && str[len - 1] == '-' && str[len - 2] != '.' && !is_dir_sep(str[len - 2])) {
        // *.?-
        if (str[len - 3] == '.')
...
static char *uncompressed_name(const char *src_name, const size_t src_len)
{
    static const struct {
        ...
        { ".xz", "" },      { ".txz", ".tar" }, // .txz abbreviation for .txt.gz is rare.
        { ".lzma", "" },
#ifdef __DJGPP__
        { ".lzm", "" },
#endif
...
{
    char *mem = NULL;
    message_warning(_("%s: File already has '%s' suffix, skipping"), tuklib_mask_nonprint(src_name),
...
extern void suffix_set(const char *suffix)
{
    ...
    if (suffix[0] == '\0' || has_dir_sep(suffix))
        message_fatal(_("%s: Invalid filename suffix"), tuklib_mask_nonprint(suffix));
