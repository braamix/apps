...
static void parse_memlimit(const char *name, const char *name_percentage, const char *str,
                           bool set_compress, bool set_decompress, bool set_mtdec)
{
    ...
    hardware_memlimit_set(value, set_compress, set_decompress, set_mtdec, is_percentage);
    return;
}

static void parse_block_list(const char *str_const)
{
    ...
    if (str[0] == '\0' || str[0] == ',')
        message_fatal(_("%s: Invalid argument to --block-list"), tuklib_mask_nonprint(str));
    ...
    if (count > SIZE_MAX / sizeof(block_list_entry) - 1)
        message_fatal(_("%s: Too many arguments to --block-list"), tuklib_mask_nonprint(str));

    // Allocate memory to hold all the sizes specified.
    // If --block-list was specified already, its value is forgotten.
    free(opt_block_list);
    opt_block_list = (block_list_entry *)xmalloc((count + 1) * sizeof(block_list_entry));
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (str[0] == '\0') {
            ...
            opt_block_list[i].size = str_to_uint64("block-list", str, 0, UINT64_MAX);
...
static void parse_real(args_info *args, int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, short_opts, long_opts, NULL)) != -1) {
        ...
        case OPT_MEM_COMPRESS:
            parse_memlimit("memlimit-compress", "memlimit-compress%", optarg, true, false, false);
            break;

        // --memlimit-decompress
        case OPT_MEM_DECOMPRESS:
            parse_memlimit("memlimit-decompress", "memlimit-decompress%", optarg, false, true,
                           false);
            break;

        // --memlimit-mt-decompress
        case OPT_MEM_MT_DECOMPRESS:
            parse_memlimit("memlimit-mt-decompress", "memlimit-mt-decompress%", optarg, false,
                           false, true);
            break;

        // --memlimit
        case 'M':
            parse_memlimit("memlimit", "memlimit%", optarg, true, true, true);
            break;
        ...
        case OPT_ROBOT:
            ...
            (void)0; // locale: not used on Braam
            break;
        ...
        case OPT_FILTERS8:
        case OPT_FILTERS9:
            coder_add_block_filters(optarg, (size_t)(c - OPT_FILTERS));
            break;
        ...
        case OPT_ARM64:
            coder_add_filter(LZMA_FILTER_ARM64, options_bcj(optarg));
            break;
        ...
        case OPT_RISCV:
            coder_add_filter(LZMA_FILTER_RISCV, options_bcj(optarg));
            break;
        ...
        case 'F': {
            ...
            static const struct {
                ...
                { "auto", FORMAT_AUTO }, { "xz", FORMAT_XZ },
                { "lzma", FORMAT_LZMA }, { "alone", FORMAT_LZMA },
        ...
        case OPT_BLOCK_SIZE:
            opt_block_size = str_to_uint64("block-size", optarg, 0, LZMA_VLI_MAX);
            break;
        ...
        case OPT_FLUSH_TIMEOUT:
            opt_flush_timeout = str_to_uint64("flush-timeout", optarg, 0, UINT64_MAX);
            break;
...
extern void args_parse(args_info *args, int argc, char **argv)
{
    ...
    if (opt_mode == MODE_COMPRESS)
        message_fatal(
            _("Compression support was disabled "
              "at build time"));
#endif
    ...
    if (opt_mode != MODE_COMPRESS)
        message_fatal(
            _("Decompression support was disabled "
              "at build time"));
#endif
    ...
    if (opt_mode == MODE_COMPRESS && opt_format == FORMAT_LZIP)
        message_fatal(
            _("Compression of lzip files (.lz) "
              "is not supported"));
#endif
    ...
    if ((opt_mode != MODE_COMPRESS || opt_format != FORMAT_XZ) && opt_block_list != NULL) {
    ...
    if (opt_format == FORMAT_RAW && !suffix_is_set() && !opt_stdout &&
        (opt_mode == MODE_COMPRESS || opt_mode == MODE_DECOMPRESS)) {
        if (args->files_name != NULL)
            message_fatal(
                _("With --format=raw, "
