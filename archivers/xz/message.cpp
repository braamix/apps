...
extern void message_verbosity_increase(void)
{
    if (verbosity < V_DEBUG)
        verbosity = (enum message_verbosity)((int)verbosity + 1);
...
static void print_filename(void)
{
    if (!opt_robot && (files_total != 1 || filename != stdin_filename)) {
        ...
        if (files_total == 0)
            xz_diag_print("%s (%u)", tuklib_mask_nonprint(filename), files_pos);
        else
            xz_diag_print("%s (%u/%u)", tuklib_mask_nonprint(filename), files_pos, files_total);
...
extern void message_progress_start(lzma_stream *strm, bool is_passthru, uint64_t in_size)
{
...
static const char *progress_sizes(uint64_t compressed_pos, uint64_t uncompressed_pos, bool final)
{
    ...
    const double ratio =
        uncompressed_pos > 0 ? (double)(compressed_pos) / (double)(uncompressed_pos) : 16.0;
...
static const char *progress_speed(uint64_t uncompressed_pos, uint64_t elapsed)
{
    ...
    double speed = (double)(uncompressed_pos) / ((double)(elapsed) * (1024.0 / 1000.0));
    ...
    // Use big enough buffer to hold e.g. a multibyte decimal point.
    static char buf[16];
    snprintf(buf, sizeof(buf), "%.*f %ciB/s", speed > 9.9 ? 0 : 1, speed, unit[unit_index]);
    return buf;
}
...
static const char *progress_time(uint64_t mseconds)
{
    ...
    if (minutes >= 60) {
        ...
        minutes %= 60;
        snprintf(buf, sizeof(buf), "%" PRIu32 ":%02" PRIu32 ":%02" PRIu32, hours, minutes, seconds);
    } else {
...
static const char *progress_remaining(uint64_t in_pos, uint64_t elapsed)
{
    ...
    if (expected_in_size == 0 || in_pos > expected_in_size || in_pos < (UINT64_C(1) << 19) ||
        elapsed < 8000)
        return "";
    ...
    if (remaining <= 10) {
        ...
        snprintf(buf, sizeof(buf), "%" PRIu32 " min %" PRIu32 " s", remaining / 60, remaining % 60);
        ...
        snprintf(buf, sizeof(buf), "%" PRIu32 " h %" PRIu32 " min", remaining / 60, remaining % 60);
        ...
        // Round up to the next multiple of an hour.
        remaining = (remaining + 3599) / 3600;
        snprintf(buf, sizeof(buf), "%" PRIu32 " d %" PRIu32 " h", remaining / 24, remaining % 24);
...
extern void message_progress_update(void)
{
    ...
    xz_diag_print("\r %*s %*s   %*s %10s   %10s\r", tuklib_mbstr_fw(cols[0], 6), cols[0],
                  tuklib_mbstr_fw(cols[1], 35), cols[1], tuklib_mbstr_fw(cols[2], 9), cols[2],
                  cols[3], cols[4]);
...
static void progress_flush(bool finished)
{
    ...
    if (!finished && !progress_active && (compressed_pos == 0 || uncompressed_pos == 0))
        return;
    ...
    if (progress_automatic) {
        ...
        char *mem = NULL;
        xz_diag_print(_("%s: "), tuklib_mask_nonprint_r(filename, &mem));
        free(mem);
...
#endif
        xz_diag_print("%s: %s", progname, line);
    }
...
extern void message_mem_needed(enum message_verbosity v, uint64_t memusage)
{
    ...
    if (memlimit < (UINT32_C(1) << 20)) {
        snprintf(memlimitstr, sizeof(memlimitstr), "%s B", uint64_to_str(memlimit, 1));
    } else {
    ...
    message(v, _("%s MiB of memory is required. The limit is %s."), uint64_to_str(memusage, 0),
            memlimitstr);
...
extern void message_filters_show(enum message_verbosity v, const lzma_filter *filters)
{
    ...
    char *buf;
    const lzma_ret ret =
        lzma_str_from_filters(&buf, filters, LZMA_STR_ENCODER | LZMA_STR_GETOPT_LONG, NULL);
    if (ret != LZMA_OK)
        message_fatal("%s", message_strm(ret));

    xz_diag_print(_("%s: Filter chain: %s\n"), progname, buf);
    free(buf);
    return;
}
...
extern void message_version(void)
{
    ...
    if (opt_robot) {
        xz_diag_print("XZ_VERSION=%" PRIu32 "\nLIBLZMA_VERSION=%" PRIu32 "\n", LZMA_VERSION,
                      lzma_version_number());
    } else {
        xz_diag_print("xz (%s) %s", PACKAGE_NAME, LZMA_VERSION_STRING);
        xz_diag_print("liblzma %s\n", lzma_version_string());
    }

    tuklib_exit(E_SUCCESS, E_ERROR, verbosity != V_SILENT);
}

static void detect_wrapping_errors(int error_mask)
{
    (void)error_mask;
}
...
message_help_unused(bool long_help)
{
	...
	xz_diag_print(_("Usage: %s [OPTION]... [FILE]...\n"), progname);
...
#if defined(HAVE_ENCODER_LZMA1) || defined(HAVE_DECODER_LZMA1) || defined(HAVE_ENCODER_LZMA2) || \
    defined(HAVE_DECODER_LZMA2)
		e |= tuklib_wrapf(stdout, &wrap2,
			"\n"
			"--lzma1[=%s]\n"
			"--lzma2[=%s]\v%s",
