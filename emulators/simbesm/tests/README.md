...
`besm6_main.cpp` hardcodes its image names and reads them from the current
directory, so the script copies `data/`'s two packs and the `unix` kernel to a
...
1. The four attach messages — `tty25` on the console, `tty26` turned off, and
   the two drums created with `-n`.
