...
Task<void> edit_chset()
{
    ...
    w = CreateWin(MIDDLE, MIDDLE, 3 * 16 + 4, 16 + 6, NORMAL_TEXT_ATTR,
                  " Character Set Visualisation ", 0);
    DisplayWin(w);
...
static inline bool cache_row(const int row)
{
    if (__builtin_expect(row >= vis_cache_len, false)) {
        ...
        wchar_t **new_vis_cache =
            (wchar_t **)realloc(vis_cache, new_vis_cache_len * sizeof(*vis_cache));
        if (!new_vis_cache)
            return false;
        // clear new storage
