...
static Task<bool> CompilePattern()
{
    if (noreg)
        co_return true;
    ...
    re_syntax_options =
        RE_SYNTAX_EMACS | RE_BK_PLUS_QM | RE_CONTEXT_INDEP_ANCHORS | RE_UNMATCHED_RIGHT_PAREN_ORD;
    rexp.translate      = 0;
    word_bounds         = false;
    hex_search          = false;
...
num no_re_search_2(const char *str, const num slen, const char *buf1, const num len1,
                   const char *buf2, const num len2, const num start, const num range)
{
    const char *pos;
    char c0 = str[0];
...
Task<int> Search(int dir, offs offslim)
{
    ...
    if ((dir == FORWARD && srchpos >= offslim) || (dir == BACKWARD && srchpos <= offslim))
        co_return FALSE;
...
search_again:
    num res;
    if (noreg)
        res = no_re_search_2((char *)pattern, patlen, buf1, len1, buf2, len2, srchpos,
                             offslim - srchpos);
    else {
        offs stop = len1 + len2;
        if (dir == FORWARD)
            stop = offslim;
        res = re_search_2(&rexp, buf1, len1, buf2, len2, srchpos, offslim - srchpos, &regs, stop);
    }
    if (res == -1)
        co_return FALSE;
    if (res == -2) {
    ...
    CurrentPos = fndind;
    if (fndlen == 0 || (word_bounds && (le_isalnum(WCharLeftAt(fndind)) ||
...
Task<void> Replace()
{
    ...
    TextPoint block_end =
        (rblock ? (BlockEnd.Col() == BlockBegin.Col() ? NextLine(BlockEnd) : BlockEnd.Offset())
                : BlockEnd.Offset());

    do {
        if (key != '*' && key != '#')
            SyncTextWin(); // before possible long search
        if (!co_await Search(FORWARD, (key != '#' ? TextEnd : block_end))) {
            if (first)
                co_await NotFound();
            else if (key == '*' || key == '#') {
                if (rcnt == 0)
        ...
        if (key != '*' && key != '#') {
            ...
            hide       = oldh;
            Message(
                "Replace? Y,R-replace L-Last replace N,D-do not replace *-replace all #-in block");
            SetCursor();
        next_action:
            ...
            if (rblock) {
                ...
                for (i = 0; i < fndlen; i++) {
                    if (pcol < BlockBegin.Col() ||
                        (BlockEnd.Col() > BlockBegin.Col() && pcol >= BlockEnd.Col() &&
                         CharRel(i) != '\n') ||
                        pline > BlockEnd.Line())
                        break;
                    if (CharRel(i) == '\t')
...
Task<void> ContSearch()
{
    ...
    if (LastDir == FORWARD) {
        ...
        MoveRight();
        if (!co_await Search(FORWARD, TextEnd)) {
            MoveLeft();
            GetCol();
            co_await NotFound();
        } else
...
{
    if (co_await getstring("Search forwards: ", (char *)pattern, sizeof(pattern) - 1,
                           &SearchHistory, &patlen, "SearchHelp", " Search Help ") < 1)
        co_return;
    LastDir = FORWARD;
    LastOp  = SEARCH;
    if (!co_await CompilePattern())
        co_return;
    co_await ContSearch();
}
Task<void> StartSearchBackward()
{
    if (co_await getstring("Search backwards: ", (char *)pattern, sizeof(pattern) - 1,
                           &SearchHistory, &patlen, "SearchHelp", " Search Help ") < 1)
        co_return;
    LastDir = BACKWARD;
    LastOp  = SEARCH;
...
Task<void> StartReplace()
{
    if (View)
        co_return;
    if (co_await getstring("Search: ", (char *)pattern, sizeof(pattern) - 1, &SearchHistory,
                           &patlen, "SearchHelp", " Search Help ") < 1)
        co_return;
    if (co_await getstring("Replace: ", (char *)replace, sizeof(replace) - 1, &SearchHistory,
                           &replen, "ReplaceHelp", " Replace Help ") < 0)
        co_return;
    LastOp = REPLACE;
    if (!co_await CompilePattern())
        co_return;
