...
void SyncTextWin()
{
    ...
    static num OldBlockBeginLine = -1, OldBlockBeginCol = -1, OldBlockEndLine = -1,
               OldBlockEndCol = -1;
    ...
    if (rblock && !hide) {
        ...
        if (BlockBegin.Col() != OldBlockBeginCol || BlockBegin.Line() != OldBlockBeginLine) {
            OldBlockBeginLine = BlockBegin.Line();
            OldBlockBeginCol  = BlockBegin.Col();
            flag              = REDISPLAY_ALL;
        }
        if (BlockEnd.Col() != OldBlockEndCol || BlockEnd.Line() != OldBlockEndLine) {
...
void StatusLine()
{
    ...
    if (View) {
        if (buffer_mmapped)
            snprintf(flags, sizeof(flags), "MM R/O %c", rblock ? 'B' : ' ');
        else
    ...
    if (in_hex_mode)
        ...
        snprintf(status, sizeof(status), "Line=%-5lu Col=%-4lu", (unsigned long)(GetLine() + 1),
...
void CenterView()
{
    ...
    if (GetLine() - ScreenTop.Line() > TextWinHeight * 2 / 3) {
        ScreenTop =
            NextNLines(ScreenTop.Offset(), GetLine() - ScreenTop.Line() - TextWinHeight / 2);
        flag = REDISPLAY_ALL;
    } else if (GetLine() - ScreenTop.Line() < TextWinHeight / 3) {
        ScreenTop =
            PrevNLines(ScreenTop.Offset(), -(GetLine() - ScreenTop.Line() - TextWinHeight / 2));
        flag = REDISPLAY_ALL;
    }
...
struct menu OkMenu[] = { { "   &Ok   ", MIDDLE, FDOWN - 2 }, { NULL } };
