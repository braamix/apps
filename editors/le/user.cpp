...
Task<void> UserCommentLine()
{
    ...
    ToLineBegin();
    if (Suffix(FileName, ".cc") || Suffix(FileName, ".cpp") || Suffix(FileName, ".cxx") ||
    ...
    } else if (Suffix(FileName, ".c") || Suffix(FileName, ".h") || Suffix(FileName, ".css")) {
        if (BlockEq("//", 2)) {
            DeleteBlock(0, 2);
            if (Char() == ' ')
                DeleteBlock(0, 1);
...
Task<void> UserSetBlockBegin()
{
    ...
    if (rblock ? (CurrentPos.Line() <= BlockEnd.Line() && CurrentPos.Col() <= BlockEnd.Col())
...
Task<void> UserSetBlockEnd()
{
    ...
    if (rblock ? (CurrentPos.Line() >= BlockBegin.Line() && CurrentPos.Col() >= BlockBegin.Col())
...
Task<int> file_check(const char *fn)
{
    ...
    if (co_await le_access(fn, R_OK) == -1) {
        ...
        struct menu CreateOrNot[] = { { " C&reate ", MIDDLE - 6, 4 },
                                      { " &Cancel ", MIDDLE + 6, 4 },
                                      { NULL } };
        snprintf(msg, sizeof(msg), "The file `%s' does not exist. Create?", fn);
        switch (co_await ReadMenuBox(CreateOrNot, HORIZ, msg, " Verify ", VERIFY_WIN_ATTR,
                                     CURR_BUTTON_ATTR)) {
        case ('R'):
            co_return OK;
        default:
...
Task<int> UserSaveAs()
{
    ...
    if (co_await getstring("Save as: ", newname, sizeof(newname) - 1, &LoadHistory, NULL, NULL) >
        0) {
        if (co_await ChooseFileName(newname, sizeof(newname)) < 0)
            co_return (ERR);
...
Task<void> UserInfo()
{
    ...
    do {
...

        {
            ...
            snprintf(s, sizeof(s), "Date: %04d-%02d-%02d %02d:%02d:%02d", c.year, c.month, c.day,
                     c.hour, c.min, c.sec);
        }
        PutStr(3, cl += 1, s);
...
Task<void> UserInsertChar(char ch)
{
    if (View)
        co_return;
    if (Text && autoindent && ch == '}' && MarginSizeAt(Offset()) == -1 &&
        MarginSizeAt(PrevLine(Offset())) == stdcol) {
