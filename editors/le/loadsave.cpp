    ...
    if (fcntl(fd, F_SETLK, &Lock) == -1) {
        if (errno == EACCES || errno == EAGAIN) {
            ...
            static struct menu LockMenu[]  = { { " &Cancel ", MIDDLE - 10, FDOWN - 2 },
                                               ...
                                               { NULL } };
            static struct menu LockMenu1[] = { { " &Cancel ", MIDDLE - 5, FDOWN - 2 },
            ...
            snprintf(msg, sizeof(msg), "This file is already locked by process %ld",
                     (long)Lock1.l_pid);
            switch (co_await ReadMenuBox(LockEnforce(st.st_mode) ? LockMenu1 : LockMenu, HORIZ, msg,
                                         " Lock Error ", VERIFY_WIN_ATTR, CURR_BUTTON_ATTR)) {
            case (0):
            case ('C'):
                co_return (-1);
            case ('I'):
...
struct menu ConCan4Menu[] = { { " C&ontinue ", MIDDLE - 6, FDOWN - 2 },
...
Task<int> LoadFile(char *name)
{
    ...
    {
#ifdef HAVE_MMAP
        if ((S_ISBLK(FileMode) || S_ISCHR(FileMode)) && st.st_size <= 0) {
        ...
        if (st.st_size > 0) {
            ...
            buffer = (char *)mmap(0, mmap_len, PROT_READ | (View ? 0 : PROT_WRITE), MAP_SHARED,
                                  file, mmap_begin);
    ...
    if (co_await b_stat(name, &st) != -1) {
        ...
        if (file != -1) {
            ...
            if (FileInfo.SameFileModified(NewFileInfo)) {
                switch (co_await ReadMenuBox(ConCan4Menu, HORIZ,
                                             "The file was changed out of the editor", " Warning ",
