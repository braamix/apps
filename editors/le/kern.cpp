...
int InsertBlock(const char *block_left, num size_left, const char *block_right, num size_right)
{
    ...
    if (ptr2 > oldptr2 && size_right > 0) {
        if (ptr2 - oldptr2 > size_right ||
            memcmp(buffer + ptr2 - size_right, block_right, size_right))
            oldptr2 = ptr2;
    }
    ...
    TextPoint::CheckSplit(Offset() - size_left - MB_LEN_MAX + 1,
                          Offset() + size_right + MB_LEN_MAX - 1);
...
int Undelete()
{
    ...
    if (oldoldptr1 >= ptr1 && oldoldptr2 <= ptr2) {
        InsertBlock(buffer + ptr1, oldoldptr1 - ptr1, buffer + oldoldptr2, ptr2 - oldoldptr2);
    }
...
bool BlockEqAt(offs o, const char *s, int len)
{
    ...
    int len1 = ptr1 - o;
    return !memcmp(buffer + o, s, len1) && !memcmp(buffer + GapSize, s + len1, len - len1);
}
