...
void vclreol(void)
{
    ...
    if (destline < 0 || destline > WECHO)
        THROW(error("Internal error: vclreol"));
    i  = WCOLS - destcol;
 ...
 * to either exdelete characters from the tab or insert white space
...
int vinschar(int c)
{
    ...
    if ((!IM || !EI) && ((hold & HOLDQIK) || !value(REDRAW) || value(SLOWOPEN))) {
        ...
        vputchar(c);
        if (DEPTH(vcline) * WCOLS + !value(REDRAW) > (destline - LINE(vcline)) * WCOLS + destcol)
            return (0);
        /*
...
#ifdef IDEBUG
    if (trace) {
        fprintf(trace, "inscol %d, inssiz %d, tabstart %d, ", inscol, inssiz, tabstart);
...
void viin(int c)
{
    ...
    if (tabsize && (IM && EI) && inssiz - doomed > tabslack)
        ...
        if (inscol + doomed == tabstart) {
             ...
             * exdelete some characters of the tab before we start
             ...
             * exdelete character) won't work correctly.
             */
...
int vputchar(int c)
{
    ...
    default:
        ...
        /*
         * This is an incurs expansion a call to vcsync() dictated
