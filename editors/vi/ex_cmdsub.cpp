 ...
 *	append, args, copy, exdelete, join, move, put,
    ...
    while (co_await (*f)() == 0) {
        if (truedol >= endcore) {
            if (morelines() < 0) {
                ...
                COTHROWV(0, error("Out of memory@- too many lines in file"));
            }
        }
        nline++;
        a1 = truedol + 1;
    ...
    for (a1 = addr1; a1 <= addr2; a1++) {
        ...
        while (*cp++ = *cp1++)
            if (cp > &genbuf[LBSIZE - 2])
                COTHROW(error("Line overflow|Result line of join would be too long"));
        cp--;
    }
    strcLIN(genbuf);
    exdelete(0);
...
Task<void> move(void)
{
    ...
    adt = address(0);
    if (adt == 0)
        COTHROW(serror("%s where?|%s requires a trailing address", Command));
    newline();
    co_await move1(iscopy, adt);
    killed();
}
...
Task<void> move1(int cflag, line *addrt)
{
    ...
    if (cflag) {
        tad1 = addr1;
        ad1  = dol;
        ignore(co_await append(getcopy, ad1++));
        ad2 = dol;
    } else {
        ad2 = addr2;
    ...
    if (adt < ad1) {
        if (adt + 1 == ad1 && !cflag && !inglobal)
            COTHROW(error("That move would do nothing!"));
        dot = adt + (ad2 - ad1);
        if (++adt != ad1) {
            reverse(adt, ad1);
            reverse(ad1, ad2);
        ...
        COTHROW(error("Move to a moved line"));
    change();
    if (!inglobal)
        if (FIXUNDO) {
            if (cflag) {
...
Task<void> put(void)
{
    ...
    tad1 = dol + 1;
    ignore(co_await append(getput, addr2));
    undkind = UNDPUT;
    notecnt = cnt;
    netchange(cnt);
}
...
void shift(int c, int cnt)
{
    ...
    for (addr = addr1; addr <= addr2; addr++) {
        ...
        if (cp + strlen(dp = vpastwh(linebuf)) >= &genbuf[LBSIZE - 2])
            THROW(error("Line too long|Result line after shift would be too long"));
        CP(cp, dp);
        strcLIN(genbuf);
        putmark(addr);
    }
...
Task<void> tagfind(exbool quick)
{
    ...
    while (fne) {
        ...
        while (co_await getfile() == 0) {
            ...
            while (*cp && *lp == *cp)
                cp++, lp++;
            if ((*lp || !iswhite(*cp)) &&
                (value(TAGLENGTH) == 0 || lp - lasttag < value(TAGLENGTH))) {
            ...
            if (!*cp)
            badtags:
                COTHROW(serror("%s: Bad tags file entry", lasttag));
            lp = filebuf;
            while (*cp && *cp != ' ' && *cp != '\t') {
            ...
            *lp++ = 0;
            ...
            if (strcmp(filebuf, savedfile) || !edited) {
                ...
                if (!quick) {
                    ckaw();
                    if (chng && dol > zero)
                        COTHROW(error("No write@since last change (:tag! overrides)"));
                }
                oglobp = globp;
                strcpy(cmdbuf2, "e! ");
    ...
    if (tfcount <= 0)
        COTHROW(error("No tags file"));
    else
        COTHROW(serror("%s: No such tag@in tags file", lasttag));
}
...
{
    if (!FIXUNDO)
        THROW(error("Can't yank inside global/macro"));
    save12();
    undkind = UNDNONE;
...
Task<void> zop2(int lines, int op)
{
    ...
    case '+':
        if (addr2 == dol)
            COTHROW(error("At EOF"));
        addr2 += lines * zweight;
        if (addr2 > dol)
            COTHROW(error("Hit BOTTOM"));
        addr2++;
    default:
        addr1 = addr2;
        addr2 += lines - 1;
    ...
    if (addr2 - addr1 > 1)
        pstart();
    if (split) {
        co_await plines(addr1, split - 1, 0);
        co_await splitit();
        co_await plines(split, split, 0);
        co_await splitit();
        addr1 = split + 1;
    }
    co_await plines(addr1, addr2, 0);
}
...
Task<void> undo(exbool c)
{
    ...
    if (inglobal && inopen <= 0)
        COTHROW(error("Can't undo in global@commands"));
    if (!c)
        somechange();
    pkill[0] = pkill[1] = 0;
    change();
    if (undkind == UNDMOVE) {
         ...
         * a exdelete command and a append command.
...
exbool somechange(void)
{
    ...
    case UNDNONE:
        THROWV(0, error("Nothing to undo"));
    }
    THROWV(0, error("Nothing changed|Last undoable command didn't change anything"));
}
...
void addmac(char *src, char *dest, char *dname, struct maps *mp)
{
    ...
    if (dest && mp == arrows) {
        ...
        if (isalpha(src[0]) && src[1] || any(src[0], ":"))
            THROW(error("Too dangerous to map that"));
    } else if (dest) {
    ...
    if (dest == NOSTR) {
        ...
        if (mp[slot].cap) {
            ...
            THROW(error("Not mapped|That macro wasn't mapped"));
        }
        return;
    }
    ...
    if (msnext - mapspace + strlen(dest) + strlen(src) + strlen(dname) + 3 > MAXCHARMACS)
        THROW(error("Too much macro text"));
    CP(msnext, src);
    mp[slot].cap = msnext;
...
Task<void> cmdmac(char c)
{
    ...
    if (inglobal < 2)
        inglobal = 1;
    co_await regbuf(c, macbuf, sizeof(macbuf));
    a1 = addr1;
    a2 = addr2;
    for (ad = a1; ad <= a2; ad++) {
        globp = macbuf;
