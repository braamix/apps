...
Task<void> vmain(void)
{
    ...
    for (;;) {
        ...
        for (;;) {
        looptop:
            if (isdigit(co_await peekkey()) && co_await peekkey() != '0') {
                hadcnt = 1;
                cnt    = co_await vgetcnt();
                forbid(cnt <= 0);
            }
            if (co_await peekkey() != '"')
                break;
            ignore(co_await getkey()), c = co_await getkey();
            /*
    ...
    reread:
        ...
        do {
            ...
            if (++maphopcnt > 256)
                COTHROW(error("Infinite macro loop"));
        } while (c != op);
        ...
        case 'J':
            ...
            co_await vremote(cnt, op_join, 0);
            notenam = "join";
            vmoving = 0;
            killU();
        ...
        case ATTN:
            obeep();
            if (initev || co_await peekkey() != ATTN)
                continue;
            /* fall into... */
         ...
         *		May specify a named buffer, or the exdelete
         *		saving buffers 1-9.
         *
...
#endif
            /*
             * If previous exdelete was partial line, use an
...
Task<void> vzop(exbool hadcnt, int cnt, int c)
{
    ...
    if (state != VISUAL) {
        ...
        vclear();
        destline = WECHO;
        co_await zop2(Xhadcnt ? Xcnt : value(WINDOW) - 1, '=');
        if (state == CRTOPEN)
            putnl();
        putNFL();
        Outchar = vputchar;
