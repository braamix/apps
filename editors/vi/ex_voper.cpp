...
nocount:
     ...
     *		For C, forward to a } in column 1 (if exdelete or such)
     *		or if a move to a { in column 1.
     */
    case ']':
        if (!vglobp)
            forbid(co_await getkey() != c);
        forbid(Xhadcnt);
        vsave();
        i = lbrack(c, opf);
        getDOT();
    ...
    case '`':
        ...
        if (opf == op_move && (wdot != dot || (d == '`' && wcursor != cursor)))
            markDOT();
        if (wcursor) {
            vsave();
            getline(*wdot);
...
exbool word(Vopf op, int cnt)
{
    ...
    if (dir == 1) {
        ...
        while (wordof(which, wcursor)) {
            if (cnt == 1 && op != op_move && wcursor[1] == 0) {
                wcursor++;
                break;
            }
            if (!lnext())
        ...
        if (op != vchange || cnt > 1)
            while (!margin() && blank())
                wcursor++;
        else if (wcursor == iwc && iwdot == wdot && *iwc)
            wcursor++;
        if (op == op_move && margin())
            wcursor--;
    } else {
        if (!lnext())
            return (0);
