 ...
 * exdelete some (blank) lines from the top of the screen so that
...
void vopenup(int cnt, exbool could, int l)
{
    ...
    if (could)
         ...
         * case, e.g. squishing out the @ lines by exdelete lines
...
void vredraw(int p)
{
    ...
    if (state == HARDOPEN || splitw)
        return;
    if (p < 0 /* || p > WECHO */)
        THROW(error("Internal error: vredraw"));
...
void vreplace(int l, int cnt, int newcnt)
{
     ...
     * can insert and exdelete lines.  Thus we just type
