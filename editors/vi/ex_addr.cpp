...
void setdot1(void)
{
    ...
    if (addr1 > addr2) {
        notempty();
        THROW(error("Addr1 > addr2|First address exceeds second"));
    }
}
...
void setcount(void)
{
    ...
    cnt = getnum();
    if (cnt <= 0)
        THROW(error("Bad count|Nonzero count required"));
    addr2 += cnt - 1;
    if (addr2 > dol)
        addr2 = dol;
    nonzero();
...
{
    if (addr2 != 0)
        THROW(error("No address allowed@on this command"));
}
...
line *address(char *incurs)
{
    ...
    for (;;) {
        ...
        case '.':
            if (addr || offset)
                THROWV(0, error("Badly formed address"));
        }
        offset += lastsign;
        lastsign = 0;
        switch (c) {
        ...
        nope:
            for (;;) {
                if (c == '/') {
                    ...
                    if (addr > dol) {
                        if (value(WRAPSCAN) == 0)
                            THROWV(0, error("No match to BOTTOM|Address search hit BOTTOM without "
                                            "matching pattern"));
                        addr = zero;
                    }
                } else {
                    ...
                    if (addr < zero) {
                        if (value(WRAPSCAN) == 0)
                            THROWV(0, error("No match to TOP|Address search hit TOP without "
                                            "matching pattern"));
                        addr = dol;
                    }
                }
                if (execute(0, addr)) {
                    if (incurs && c == '?') {
                        incurs = &linebuf[LBSIZE];
                        goto doques;
                    }
                    break;
                }
                if (addr == dot)
                    THROWV(0, error("Fail|Pattern not found"));
            }
            continue;
        ...
        case '\'':
            c = markreg(getchar());
            if (c == 0)
                THROWV(0, error("Marks are ' and a-z"));
            addr = getmark(c);
            if (addr == 0)
                THROWV(0, error("Undefined mark@referenced"));
            break;
        ...
        default:
            ...
            addr += lastsign;
            if (addr < zero)
                THROWV(0, error("Negative address@- first buffer line is 1"));
            if (addr > dol)
                THROWV(0, error("Not that many lines@in buffer"));
            return (addr);
        }
    }
}
