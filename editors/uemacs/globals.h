...
/* Some global fuction declarations.  Every bound command awaits a key or a
   syscall somewhere below it, so all of them are coroutines. */
typedef Task<int> (*fn_t)(int, int);
...
/* exit() cannot unwind a coroutine, so leaving is a flag: cmd_exit_emacs()
   raises it, execute() stops, and the command loop in main() falls out. */
extern int quitting;
...
/* noticed where a key is read, acted on by checkwinsize() */
