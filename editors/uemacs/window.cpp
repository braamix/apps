 ...
 * are in.
 */
static Task<int> enter_window(struct window *wp)
{
    curwp = wp;
    curbp = wp->w_bufp;
    update_modeline();
 ...
 * Bound to "C-x 2".
 */
Task<int> cmd_split_current_window(int f, int n)
{
    struct window *wp;
    struct line *lp;
    ...
    /* xmalloc() die()d, and there is no exit from a depth here. */
    wp = (struct window *)malloc(sizeof(struct window));
    if (wp == NULL) {
        msg_printf("Cannot allocate a window");
        co_return FALSE;
    }
 ...
 */
Task<int> cmd_next_window(int f, int n)
{
    struct window *wp;
    ...
    if (!f) {
        ...
        if (wp == NULL)
            wp = window_head;
        co_return co_await enter_window(wp);
    }
 ...
 */
Task<int> cmd_previous_window(int f, int n)
{
    struct window *wp;

    if (f) /* nth from the bottom */
        co_return co_await cmd_next_window(f, -n);
 ...
 * Bound to "C-x 1".
 */
Task<int> cmd_delete_other_windows(int f, int n)
{
    struct window *wp;
    struct line *lp;
    int i;
 ...
 */
Task<int> cmd_delete_window(int f, int n)
{
 ...
 */
Task<int> cmd_grow_window(int f, int n)
{
    struct window *adjwp;
    struct line *lp;
    int i;
    ...
    if (n < 0)
        co_return co_await cmd_shrink_window(f, -n);
 ...
 * Bound to "C-x C-Z".
 */
Task<int> cmd_shrink_window(int f, int n)
{
    struct window *adjwp;
    struct line *lp;
    int i;
    ...
    if (n < 0)
        co_return co_await cmd_grow_window(f, -n);
    adjwp = adjacent_window();
    if (adjwp == NULL) {
        msg_printf("Only one window");
        co_return FALSE;
 ...
 */
Task<int> cmd_move_window_up(int f, int n)
{
 ...
 */
Task<int> cmd_move_window_down(int f, int n)
{
    co_return co_await cmd_move_window_up(f, -n);
}
 ...
 */
Task<int> cmd_resize_window(int f, int n)
{
    if (f == FALSE)
        co_return TRUE;
    if (curwp->w_ntrows == n)
        co_return TRUE;
    co_return co_await cmd_grow_window(TRUE, n - curwp->w_ntrows);
}
 ...
 * "M-C-V".
 */
Task<int> cmd_scroll_next_up(int f, int n)
{
    co_await cmd_next_window(FALSE, 1);
    co_await cmd_previous_page(f, n);
    co_await cmd_previous_window(FALSE, 1);
    co_return TRUE;
}

Task<int> cmd_scroll_next_down(int f, int n)
{
    co_await cmd_next_window(FALSE, 1);
    co_await cmd_next_page(f, n);
    co_await cmd_previous_window(FALSE, 1);
    co_return TRUE;
}
 ...
 * closed in between.
 */
Task<int> cmd_save_window(int f, int n)
{
    saved_window = curwp;
    co_return TRUE;
}

Task<int> cmd_restore_window(int f, int n)
{
    struct window *wp;
 ...
 */
Task<int> cmd_redraw_display(int f, int n)
{
 ...
 */
Task<int> cmd_clear_and_redraw(int f, int n)
{
    if (f == FALSE)
        screen_garbage = TRUE;
    else {
 ...
 */
Task<int> cmd_change_screen_size(int f, int n)
{
 ...
 * int n;		numeric argument
 */
Task<int> cmd_change_screen_width(int f, int n)
{
    /* if the command defaults, assume the largest */
    if (f == FALSE)
        n = term.t_mcol;
