...
static struct magic_replacement
    magic_replace_pattern[NPAT]; /* the replacement, compiled the same way */

static int amatch(struct magic *mcptr, int direct, struct line **pcwline, int *pcwoff);
static Task<int> readpattern(char *prompt, char *apat, int srch);
static Task<int> replaces(int kind, int f, int n);
 ...
 */
Task<int> cmd_search_forward(int f, int n)
{
    int status = TRUE;
     ...
     */
    if (n < 0)
        co_return co_await cmd_search_reverse(f, -n);
     ...
     * is just fine).
     */
    if ((status = co_await readpattern("Search", &search_pattern[0], TRUE)) == TRUE) {
        do {
 ...
 * int f, n;		default flag / numeric argument
 */
Task<int> cmd_hunt_forward(int f, int n)
{
    int status = TRUE;

    if (n < 0) /* search backwards */
        co_return co_await cmd_hunt_backward(f, -n);
 ...
 * int f, n;		default flag / numeric argument
 */
Task<int> cmd_search_reverse(int f, int n)
{
    int status = TRUE;
     ...
     */
    if (n < 0)
        co_return co_await cmd_search_forward(f, -n);
     ...
     * is just fine).
     */
    if ((status = co_await readpattern("Reverse search", &search_pattern[0], TRUE)) == TRUE) {
        do {
            if ((magical && curwp->w_bufp->b_mode & MDMAGIC) != 0)
 ...
 * int f, n;		default flag / numeric argument
 */
Task<int> cmd_hunt_backward(int f, int n)
{
    int status = TRUE;

    if (n < 0)
        co_return co_await cmd_hunt_forward(f, -n);
...
static Task<int> readpattern(char *prompt, char *apat, int srch)
{
     ...
     */
    if ((status = co_await ask_string_until(tpat, tpat, NPAT, meta_char)) == TRUE) {
        strcpy(apat, tpat);
...
void savematch(void)
{
    ...
    ptr = matched_text = (char *)malloc(match_bytes + 1);
 ...
 */
Task<int> cmd_replace_string(int f, int n)
{
    co_return co_await replaces(FALSE, f, n);
}
 ...
 * int n;		# of repetitions wanted
 */
Task<int> cmd_query_replace_string(int f, int n)
{
    co_return co_await replaces(TRUE, f, n);
}
...
static Task<int> replaces(int kind, int f, int n)
{
     ...
     */
    if ((status = co_await readpattern((char *)(kind == FALSE ? "Replace" : "Query replace"),
                                       &search_pattern[0], TRUE)) != TRUE)
        co_return status;

    /* Ask for the replacement string.
     */
    if ((status = co_await readpattern("with", &replace_pattern[0], FALSE)) == ABORT)
        co_return status;
    ...
    while ((f == FALSE || n > nummatch) && (nlflag == FALSE || nlrepl == FALSE)) {
        ...
        pprompt:
            msg_puts(tpat);
        qprompt:
            co_await update_now(); /* show the proposed place to change */
            c = co_await tgetc();  /* and input */
            ...
            case 'u': /* undo last and re-prompt */
                 ...
                 */
                --numsub;
                backchar(FALSE, previous_match_bytes);
                match_line   = curwp->w_dotp;
                match_offset = curwp->w_doto;
                goto pprompt;
