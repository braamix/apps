 ...
 */
Task<int> cmd_reverse_incremental_search(int f, int n)
{
    ...
    if (!(co_await isearch(f, -n))) { /* Call ISearch backwards       */ /* If error in search: */
 ...
 */
Task<int> cmd_incremental_search(int f, int n)
{
    struct line *curline; /* Current line on entry              */
    int curoff;           /* Current offset on entry            */
    ...
    if (!(co_await isearch(f, n))) { /* Call ISearch forwards        */ /* If error in search: */
        curwp->w_dotp = curline; /* Reset the line pointer             */
        curwp->w_doto = curoff;  /*  and the offset to original value  */
        curwp->w_flag |= WFMOVE; /* Say we've moved                    */
        co_await update();
...
start_over:
    ...
    c = keycode_to_char(expc = co_await get_char());      /* Get the first character    */
    if ((c == IS_FORWARD) || (c == IS_REVERSE)) {         /* Reuse old search string?   */
        ...
        c      = keycode_to_char(expc = co_await get_char()); /* Get another character      */
    }
    ...
    for (;;) { /* ISearch per character loop */
        ...
        if (expc == meta_char) /* Want to quit searching?    */
            co_return TRUE;    /* Quit searching now         */

        switch (c) {         /* dispatch on the input char */
        case IS_ABORT:       /* If abort search request    */
            co_return FALSE; /* Quit searching again       */
        ...
        case IS_FORWARD:                          /* If forward search          */
            ...
            c      = keycode_to_char(expc = co_await get_char()); /* Get the next char          */
        ...
        default:                                              /* All other chars            */
            if (c < ' ') { /* Is it printable?             */ /* Nope.                      */
                ...
                co_return TRUE;                               /* And return the last status */
            }
        } /* Switch */
        ...
        if (cpos >= NPAT) { /* too many chars in string?  */ /* Yup.  Complain about it    */
            ...
            co_return TRUE; /* Return an error            */
        }
        ...
        if (!status) {                                       /* If we lost last time       */
            ...
            if (!(status = checknext(c, search_pattern, n))) /* See if match         */
