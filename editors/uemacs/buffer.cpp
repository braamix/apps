 ...
 */
Task<int> cmd_select_buffer(int f, int n)
{
    struct buffer *bp;
    int s;
    char bufn[NBUFN];

    s = co_await ask_string("Use buffer: ", bufn, NBUFN);
    if (s != TRUE)
        co_return s;
    if ((bp = find_buffer(bufn, TRUE, 0)) == NULL)
        co_return FALSE;
    co_return co_await swbuffer(bp);
}
 ...
 */
Task<int> cmd_next_buffer(int f, int n)
{
 ...
 */
Task<int> swbuffer(struct buffer *bp)
{
    ...
    if (curbp->b_active != TRUE) { /* buffer not active yet */
        ...
        co_await readin(curbp->b_fname, TRUE);
 ...
 */
Task<void> shown_buffer_changed(void)
{
    co_await execute(META | SPEC | 'X', FALSE, 1);
}
 ...
 */
Task<int> cmd_delete_buffer(int f, int n)
{
    struct buffer *bp;
    int s;
    char bufn[NBUFN];

    s = co_await ask_string("Kill buffer: ", bufn, NBUFN);
    if (s != TRUE)
        co_return s;
    if ((bp = find_buffer(bufn, FALSE, 0)) == NULL) /* Easy if unknown.     */
        co_return TRUE;
    if (bp->b_flag & BFINVS) /* Deal with special buffers        */
        co_return TRUE;      /* by doing nothing.    */
    co_return co_await destroy_buffer(bp);
}

/*
 * kill the buffer pointed to by bp
 */
Task<int> destroy_buffer(struct buffer *bp)
{
    struct buffer *bp1;
    struct buffer *bp2;
    int s;
    ...
    s = co_await clear_buffer(bp); /* Blow text away.      */
    if (s != TRUE)
        co_return s;
 ...
 */
Task<int> cmd_name_buffer(int f, int n)
{
...
ask:
    if (co_await ask_string("Change buffer name to: ", bufn, NBUFN) != TRUE)
        co_return FALSE;
...
Task<int> makelist(int iflag)
{
    ...
    s = co_await clear_buffer(list_buffer); /* Blow old text away   */
    if (s != TRUE)
        co_return s;
 ...
 */
Task<int> cmd_list_buffers(int f, int n)
{
    int s;

    s = co_await makelist(f);
    if (s != TRUE)
        co_return s;

    if (curwp->w_bufp != list_buffer) {
        s = co_await swbuffer(list_buffer);
        if (s != TRUE)
            co_return s;
    }
 ...
 * looks good.
 */
Task<int> clear_buffer(struct buffer *bp)
{
    struct line *lp;
    int s;
    ...
    if ((bp->b_flag & BFINVS) == 0   /* Not scratch buffer.  */
        ...
        && (s = co_await ask_yesno("Discard changes")) != TRUE)
        co_return s;
 ...
 */
Task<int> cmd_unmark_buffer(int f, int n)
{
    curbp->b_flag &= ~BFCHG;
    curwp->w_flag |= WFMODE;
    co_return TRUE;
