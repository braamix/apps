...
/* Signed: a key code is an int, and C++ refuses to narrow 0x80000000. */
#define SPEC ((int)0x80000000) /* special key (function keys)  */
 ...
 *
 * Upstream keyed on device and inode as well, so a file replaced by writing
 * a temporary and renaming it over - which is what git and patch do - was
 * caught even at the same size.  Braam's stat answers a kind, a size and a
 * millisecond mtime and there is no inode, so that case is now caught only
 * when one of those three moved.
 */
...
struct filestate {
    ...
    u32 fs_kind;
    u64 fs_size;
    u64 fs_mtime; /* ms since the epoch, 0 if none */
};
...
struct key_tab {
    int k_code;                  /* Key code */
    Task<int> (*k_fp)(int, int); /* Routine to handle it */
};

/* Structure for the name binding table. */
struct name_bind {
    char *n_name;                  /* name of function key */
    Task<int> (*n_func)(int, int); /* function name is bound to */
};
