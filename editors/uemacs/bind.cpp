...
Task<int> cmd_help(int f, int n)
{
    ...
    if (bp == NULL) {
        fname = co_await lookup_file(pathname[1], TRUE);
        if (fname == NULL) {
    ...
    if (bp == NULL) {
        if (co_await getfile(fname, FALSE) == FALSE)
            co_return FALSE;
    } else
 ...
 */
static Task<int> build_binding_list(char *match)
{
    ...
    if (bp == NULL || co_await clear_buffer(bp) == FALSE) {
    ...
    if (curwp->w_bufp != bp && co_await swbuffer(bp) != TRUE)
        co_return FALSE;
 ...
 */
Task<int> cmd_describe_bindings(int f, int n)
{
    co_return co_await build_binding_list(NULL);
}
 ...
 * Bound to "M-A".
 */
Task<int> cmd_apropos(int f, int n)
{
    char match[NSTRING];
    int status;

    status = co_await ask_string("Apropos string: ", match, NSTRING - 1);
    if (status != TRUE)
        co_return status;
    co_return co_await build_binding_list(match);
}

Task<int> cmd_describe_key(int f, int n)
{                         /* describe the command for a certain key */
    ...
    cmdstr(c = co_await getckey(FALSE), &outseq[0]);
 ...
 */
Task<int> cmd_bind_to_key(int f, int n)
{
    ...
    c = co_await getckey((kfunc == cmd_meta_prefix) || (kfunc == cmd_ctlx_prefix) ||
                         (kfunc == cmd_universal_argument) || (kfunc == cmd_abort_command));
    ...
    if (kfunc == cmd_meta_prefix || kfunc == cmd_ctlx_prefix || kfunc == cmd_universal_argument ||
        kfunc == cmd_abort_command) {
 ...
 * int f, n;		command arguments [IGNORED]
 */
Task<int> cmd_unbind_key(int f, int n)
{
    ...
    c = co_await getckey(FALSE); /* get a command sequence */
 ...
 */
Task<unsigned int> getckey(int mflag)
{
    ...
    if (executing_command_line) {
        co_await macarg(tok); /* get the next token */
        co_return stock(tok);
    }
 ...
 */
Task<int> startup(char *sfname)
{
    char *fname; /* resulting file name to execute */

    /* look up the startup file */
    if (*sfname != 0)
        fname = co_await lookup_file(sfname, TRUE);
    else
        fname = co_await lookup_file(pathname[0], TRUE);
    ...
    co_return co_await dofile(fname);
}
 ...
 */
Task<char *> lookup_file(char *fname, int try_home)
{
    ...
    if (co_await file_open_read(fname) == FIOSUC) {
        co_await file_close();
        co_return fname;
    }
