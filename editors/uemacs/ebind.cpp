 ...
 *
 *	It was ebind.h, included once by main.c.  A table of coroutine
 *	addresses has to live in a translation unit that does not also call
 *	them: taking the address of one that is only declared, where the same
 *	file calls it, gets a bitcast thunk that drops the arguments.  names.c
 *	always had its own file for the same table shape.
 */
 ...
 * control-X commands.
 */
struct key_tab keytab[NBINDS] = { { CONTROL | 'A', cmd_beginning_of_line },
