 ...
 */
Task<int> cmd_insert_space(int f, int n)
{
    insert_char(n, ' ');
    backchar(f, n);
    co_return TRUE;
...
static int insert_byte(int n, int c)
{
    ...
    if (lp1->l_used + n > lp1->l_size) { /* Hard: reallocate     */
        ...
        lp2->l_used += n;
        memmove(lp1->l_text + doto + n, lp1->l_text + doto, lp1->l_used - doto - n);
    }
 ...
 */
Task<int> putctext(char *iline)
{
    int status;

    /* delete the current line */
    curwp->w_doto = 0; /* starting at the beginning of the line */
    status        = co_await cmd_kill_to_end_of_line(TRUE, 1);
    if (status != TRUE)
        co_return status;
...
Task<int> cmd_yank(int f, int n)
{
    ...
    if (kill_head == NULL)
        co_return TRUE; /* not an error, just nothing */
