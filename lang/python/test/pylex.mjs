// The tokenizer, measured against CPython's. Every golden under lex/ was
// written by tools/mklex.py out of CPython's own tokenize module, so a
...
for (const name of readdirSync(DIR).filter((f) => f.endsWith(".py")).sort()) {
    ...
    put("/tmp/c.py", source);
    const r = run("--dump-tokens /tmp/c.py");
    ran++;
    ...
    if (!existsSync(exp)) die(`no golden at ${exp} — run tools/mklex.py`);
...
if (opt.bless) {
    console.error(`pylex: blessed ${ran} cases`);
    process.exit(0);
}
if (bad) {
    console.error(`pylex: ${bad} of ${ran} cases differ`);
    process.exit(1);
}
ok(`${ran} sources token for token, ${upstream} upstream tests tokenized`);
