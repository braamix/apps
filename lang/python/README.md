...
`_pyio.py`, which opens by importing `io` and so could not be the floor.
...
| 1 | other syntax — PEP 798 | 1 | 1 | 1 | 1 | 2 | 2 | 44 | 43 | 43 | 43 | 43 | 34 | 28 | not taken yet |
...
Both clones live under [tmp/](tmp/) and neither is committed; what is taken
from them is.
...
and it was the ruler for the language core. It is MIT ([LICENSE](LICENSE)).
...
- **[tmp/cpython/Lib/test/](tmp/cpython/Lib/test/)** — 519 entries, the real
  ...
  `string_tests.py` are shared behaviour suites several of them mix in. These
  are unforgiving in a way MicroPython's are not.
- **[tmp/cpython/Lib/](tmp/cpython/Lib/)** — the standard library, most of it
  ...
  worse. So the floor is implemented here and the rest taken verbatim.

**The library decides the syntax**, because it is written in the Python of its
own day: `dataclasses.py` has 92 f-strings and a `match` statement,
`typing.py` fifteen PEP 695 generics, `argparse.py` nine `lazy` imports. Each
had to land before the module that needed it could be compiled at all.

**The host's CPython is 3.14** — `/opt/homebrew/bin/python3`, first on `PATH`
— and it wrote most of the goldens. `tools/pyref.py` is where the `mk*` tools
get their interpreter: `$PYTHON`, or `python3` on `PATH`; the manifest's `exp`
column and [test/goldens.txt](test/goldens.txt) record which one wrote each
golden, and a golden is not rewritten by another one without `--regen`. The
`lazy` cases are the exception: 3.14 has no PEP 810, so they come from the
clone built out of tree at `tmp/cpython-build/`.

**The version tracked is 3.14's language**, which is what `sys.version_info`
says, plus two things from CPython's main branch that the library written
there already uses: PEP 810's `lazy` imports, and a `+` before a number in a
pattern. Nothing else from 3.15 is taken until a library module or a test
needs it; PEP 798's unpacking in comprehensions is the first such thing
`pycases.mjs --survey` names, in `test_listcomps.py`.
...
1. **The VM is a driver, not a coroutine.** `emulators/simbesm`'s shape. Only
   ...
   and hands the answer back. A `co_await` is a call and not a tail call, so
   an interpreter loop that awaited would grow the native stack until the
   process trapped. *Why the VM is a driver*, below, works it through.
2. **The VM never recurses for a Python call.** A call pushes a frame and the
   ...
   parks its state in a `ContObj` ([call.h](src/call.h)) and returns it; the
   VM records the continuation on the frame it pushes, and `Return` brings the
...
100 MB of linear memory (`../../../braam-core/src/kernel/sysabi.h`); a 128 KiB
shadow stack; `PROC_TASKS = 8`.
...
**Collection is precise mark-and-sweep.** The roots are exhaustive and
explicit — the frame stack, each frame's locals and value stack, the module
globals, the intern table, the type objects, and an RAII `Root` chain for C++
code holding a value across an allocation. Mark from those, sweep the `next`
list, collect on bytes-allocated pressure. *Why every root is explicit*, below,
says why there is no other option here.
...
true of a comparison a sort has to make and of a finalizer the sweep owes:
each is a state machine the VM drives, never a nested interpreter.
...
- A test or a library module copied from upstream is copied **byte for byte**.
  Its provenance — the upstream path and the commit it came from — goes in
  the manifest beside it ([test/manifest.txt](test/manifest.txt),
  [test/cpython.txt](test/cpython.txt), [lib/manifest.txt](lib/manifest.txt)),
  never in the file. A module that needs something this interpreter has not got
  waits; it is not trimmed to fit.
- A golden is written by the CPython [tools/pyref.py](tools/pyref.py) finds,
  and recorded in [test/goldens.txt](test/goldens.txt). Another interpreter
  does not overwrite it without `--regen`.
